...
import { money, moneyRound } from "../src/lib/format.ts";
...
test("a negative sits outside the peso sign, where it can't be skimmed past", () => {
  // brand.currency.symbol + "-1.25" reads as a currency code followed by a number. The minus is the
  // whole point on a costing screen, so it goes first.
  assert.equal(money(-1.25), "-" + brand.currency.symbol + "1.25");
  assert.equal(moneyRound(-1234.6), "-" + brand.currency.symbol + "1,235");
});

test("a headline figure drops the centavos", () => {
  assert.equal(moneyRound(1234.4), brand.currency.symbol + "1,234");
  assert.equal(moneyRound(1234.6), brand.currency.symbol + "1,235");
});
