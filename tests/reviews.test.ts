...
import { displayName, prepareRelayedReview } from "../src/lib/reviews.ts";
import { shopMidday } from "../src/lib/format.ts";
 ...
 * pinned to the shop's zone. Get that wrong by eight hours and every relayed review
 * is filed on the wrong day — quietly, and only visibly weeks later when the
 * timeline reads oddly.
 */

const ok = (day: string) => {
  const at = shopMidday(day);
  assert.ok(at, `${day} should be a real date`);
  return at;
};

test("a typed day is stored as the middle of that day in the shop's zone", () => {
...
test("the stored instant reads back as the day that was typed", () => {
  const fmt = new Intl.DateTimeFormat("en-CA", { timeZone: brand.timeZone });
...
test("a day that does not exist is refused, not rolled forward", () => {
  assert.equal(shopMidday("2026-02-31"), null);
  assert.equal(shopMidday("2026-13-01"), null);
});

test("anything that is not a plain date is refused", () => {
  for (const bad of ["", "today", "4/3/2026", "2026-3-4", "2026-03-04T10:00:00Z"]) {
    assert.equal(shopMidday(bad), null);
  }
});
