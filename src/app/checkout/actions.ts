...
type PlaceOrderInput = {
  // `name` is the browser's copy, used only to name a sold-out product back to
...
export async function placeOrder(
  ...
  const mealIds = input.items.map((i) => i.productId);
  const { data: products, error: mealsError } = await supabase
    .from("products")
    .select("id, name, price")
    .in("id", mealIds);
  if (mealsError || !products) {
    return { error: "Could not verify menu prices." };
  }

  const priceById = new Map(products.map((m) => [m.id, Number(m.price)]));
  const nameById = new Map(products.map((m) => [m.id, m.name as string]));
  // Name the product. "One of the items in your cart" makes the customer open
  // every line to work out which — and a sold-out product is annoying enough
  ...
  const subtotal = input.items.reduce(
    (sum, i) => sum + priceById.get(i.productId)! * i.qty,
    0
  );
  ...
  if (input.scheduledFor) {
    ...
    // collects costs the shop one product it can still sell to the next person
    // in the queue; a scheduled one costs materials bought and prep time
  ...
  // tab, a back button or a crafted request all reach this line with a product
  // whose materials ran out ten minutes ago. Selling food that cannot be
  // cooked costs a refund and a customer.
  const makeable = await loadAvailability();
  const short = input.items.filter((i) => {
    const n = makeable.get(i.productId);
    return n !== undefined && n < i.qty;
  });
  if (short.length > 0) {
    const names = short
      .map((i) => nameById.get(i.productId) ?? "an item")
      .join(", ");
    return {
  ...
  const { error: linesError } = await supabase.from("order_lines").insert(
    input.items.map((i) => ({
      ...
      qty: i.qty,
      price_at_sale: priceById.get(i.productId)!,
    }))
  );
  if (linesError) {
