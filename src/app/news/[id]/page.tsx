...
export default async function AnnouncementPage({ params }: PageProps<"/news/[id]">) {
  ...
  const badge: Record<typeof row.kind, { label: string; chip: string }> = {
    promo: { label: "Promo", chip: "bg-brand-700 text-paper-50" },
    news: { label: "News", chip: "bg-ok-600 text-paper-50" },
    coming_soon: { label: "Coming soon", chip: "bg-accent-200 text-ink-950" },
    dine_in: { label: "Dine-in special", chip: "bg-ink-950 text-accent-200" },
  };
  ...
  return (
     ...
     */
    <main className="under-nav relative flex-1 overflow-hidden bg-paper-100">
      <div
        aria-hidden
      ...
      <Chili
        aria-hidden
        className="pointer-events-none absolute -right-20 -top-16 h-80 w-80 rotate-12 text-brand-700/[0.07]"
      />
      <NoodleBowl
        aria-hidden
      ...
      <article className="relative mx-auto max-w-3xl px-6 py-14">
        <Link
          href="/news"
          className="text-xs font-black uppercase tracking-widest text-ink-900/50 transition-colors hover:text-brand-700"
        >
          ← News &amp; promos
        </Link>

        <div className="mt-6 rounded-3xl border-4 border-ink-950 bg-paper-50 p-6 shadow-[8px_8px_0_0_theme(colors.ink.950)] sm:p-10">
        <p className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs font-semibold uppercase tracking-widest text-ink-900/45">
        ...
        {when && (
          <p className="mt-3 inline-block rounded-xl bg-ink-950/5 px-4 py-2 text-sm font-bold text-ink-900/70">
            {when}
          </p>
        )}
        ...
        {row.body && (
          ...
          <div className="mt-8 whitespace-pre-line text-lg leading-relaxed text-ink-900/85">
            {row.body}
          </div>
        )}
        ...
        <div className="mt-10 flex flex-wrap gap-3 border-t-2 border-ink-950/10 pt-8">
          <Link
            href="/menu"
            className="rounded-full bg-ink-950 px-7 py-3.5 font-bold text-accent-200 transition-transform hover:scale-105"
          >
            See the menu →
          </Link>
          <Link
