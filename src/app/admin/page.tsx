...
const dayLabel = new Intl.DateTimeFormat(brand.locale, {
    day: "numeric",
});
const dayCaption = new Intl.DateTimeFormat(brand.locale, {
    month: "short",
  day: "numeric",
});
...
export default async function AdminDashboard({
  ...
  // costing existed is one of these, and so is any order of a product with no
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatTile
            ...
            value={money(kept(inRange))}
            detail={
              uncosted(inRange) > 0
                ? `Best case — ${uncosted(inRange)} order${
                    uncosted(inRange) === 1 ? " has" : "s have"
                  } no cost recorded`
                : "After materials, before everything else"
            }
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatTile
            ...
            detail={
              deliveryFeesMonth > 0
                ? `${money(deliveryFeesMonth)} in fees this month`
        ...
        {waitingLeads > 0 && (
          <Link
            href="/admin/inbox"
            className="mt-4 mr-3 inline-block rounded-full bg-accent-200 px-6 py-3 text-sm font-bold text-ink-950 transition-transform hover:scale-105"
          >
        ...
        {needsAction.length > 0 && (
          <Link
            href="/admin/orders"
            className="mt-4 inline-block rounded-full bg-brand-700 px-6 py-3 text-sm font-bold text-paper-50 transition-transform hover:scale-105"
          >
      ...
      <section>
        ...
        <div className="mt-5 rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
          <ColumnChart data={salesByDay} hue="money" format="money" />
        </div>
      </section>
      ...
      <Link
        ...
        className="flex flex-wrap items-center gap-3 rounded-2xl bg-paper-100 px-5 py-4 ring-1 ring-ink-950/10 transition-colors hover:bg-paper-200"
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-sm text-ink-900/60">
            All of it lives in Insights, with a date range you can set.
          </span>
        </span>
        <span className="font-bold text-brand-700">Open Insights →</span>
      </Link>
      ...
      <section>
        ...
        {orders.length === 0 ? (
          <p className="mt-4 rounded-2xl border-2 border-dashed border-brand-300 bg-paper-100 p-6 text-sm text-ink-900/70">
            No orders yet.
          </p>
        ) : (
          <ul className="mt-5 flex flex-col gap-2">
            {orders.slice(0, 8).map((o) => (
              <li
                key={o.id}
                className="flex items-center justify-between gap-4 rounded-2xl bg-paper-100 px-5 py-3 ring-1 ring-ink-950/10"
              >
...
{formatDateTime(o.created_at)}
                ...
                <span className="flex items-center gap-4">
                  <span className="text-xs font-bold uppercase tracking-wide text-ink-900/70">
                    {o.status}
                  </span>
                  <span className="font-display font-black text-ink-950">
                    {money(Number(o.revenue))}
                  </span>
                </span>
              </li>
            ))}
...
async function ServiceBoard({
  ...
  // Names for the ids `loadAvailability` returns. Only the products actually on
  // the menu — a hidden product running out is nobody's problem this shift.
  const { data: products } = await supabase
    .from("products")
    .select("id, name")
    .eq("is_public", true);
  const nameById = new Map(
    ((products ?? []) as { id: string; name: string }[]).map((m) => [m.id, m.name])
  );
