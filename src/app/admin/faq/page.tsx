...
export default async function AdminFaqPage() {
    ...
    return (
      <div className="rounded-3xl bg-accent-50 p-8 ring-1 ring-accent-200/40">
        ...
        <p className="mt-2 max-w-xl text-sm text-ink-900/70">
          ...
          {brand.name} gives — and choose which of them the homepage prints.
        </p>
        <p className="mt-3 rounded-xl bg-paper-50 px-4 py-2 font-mono text-xs text-ink-900/70">
          {error.message}
        </p>
      </div>
    );
  ...
  for (const messages of byThread.values()) {
    for (let i = 1; i < messages.length; i++) {
      ...
      // Both indexes are inside the loop's own bounds; saying so is cheaper
      // than trusting the reader to re-derive it.
      if (
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm text-ink-900/60">
          Anything you write here, Ask {brand.name} says word for word — and it
      ...
      <div className="flex flex-wrap items-center gap-6 rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
        ...
        <p className="min-w-48 flex-1 text-sm text-ink-900/60">
