...
function stamp(): string {
  const parts = new Intl.DateTimeFormat(brand.locale, {
        year: "numeric",
    month: "2-digit",
    day: "2-digit",
    hour: "2-digit",
...
async function loadRecipeTables() {
  ...
  const [ing, bat, batIng, mea, meaIng, meaComp] = await Promise.all([
    ...
    supabase.from("production_runs").select("*").order("name"),
  ...
  return {
    materials: (ing.data ?? []) as Material[],
    production_runs: (bat.data ?? []) as ProductionRun[],
    productionRunMaterials: (batIng.data ?? []) as BatchIngredient[],
    products: (mea.data ?? []) as Product[],
    productMaterials: (meaIng.data ?? []) as MealIngredient[],
    productComponents: (meaComp.data ?? []) as MealComponent[],
  };
}
...
export async function GET(request: NextRequest) {
  ...
  if (kind === "order-lines.csv") {
    const [{ data: lines }, { data: orders }, { data: products }] = await Promise.all([
    ...
    const rows = ((lines ?? []) as Record<string, unknown>[])
      .map((l) => {
        ...
        return [
          ...
          l.order_id,
          o?.status ?? "",
          mealById.get(String(l.product_id)) ?? "(deleted product)",
          qty,
          price,
          qty * price,
        ];
  ...
  if (kind === "product-costs.csv") {
    const t = await loadRecipeTables();
    const batchCosts = costBatches(t.production_runs, t.productionRunMaterials, t.materials);
    ...
    return file(
      toCsv(
        ...
        [...mealCosts.values()].map((mc) => {
          ...
          return [
            mc.product.name,
            mc.product.is_public ? "yes" : "no",
            mc.product.is_available ? "yes" : "no",
            mc.product.price,
