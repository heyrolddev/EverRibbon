...
export default function AdminError({
  ...
  return (
    <div className="rounded-3xl bg-paper-100 p-8 ring-1 ring-ink-950/10">
      ...
      {error.digest && (
        <p className="mt-4 rounded-xl bg-ink-950/5 px-4 py-3 font-mono text-xs text-ink-900/70">
          Reference: {error.digest}
          <span className="mt-1 block font-sans text-ink-900/50">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        <Link
          href="/admin/counter"
          className="rounded-xl bg-ok-600 px-5 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-ok-700"
        >
          Go to the counter
        </Link>
        <Link
