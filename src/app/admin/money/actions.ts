...
export async function saveFixedCost(input: {
  ...
  await log(`Set "${label}" at ${money(input.amount)} a month`, owner.profile?.id ?? null);
  done();
  return { error: null };
}
...
export async function startCashTracking(openingAmount: number): Promise<Result> {
  ...
  if (error) return { error: error.message };
  await log(
    `Started counting cash from ${money(openingAmount)}`,
...
export async function addCashEntry(input: {
  ...
  await log(
    `Cash ${input.type === "in" ? "in" : "out"} ${money(input.amount)}${
...
export async function addReceivable(input: {
  ...
  if (error) return { error: error.message };
  await log(
    `Utang: ${input.customer.trim()} ${money(input.amount)}`,
    viewer!.profile?.id ?? null
  );
  done();
  return { error: null };
...
export async function collectReceivable(input: {
  ...
  await log(
    `Collected ${money(input.amount)} from ${row.customer ?? "a customer"}`,
    viewer!.profile?.id ?? null
  );
  done();
  return { error: null };
