...
export async function endShift(input: {
   ...
   * usually missing. Without it "took ${moneyRound(3400)} in cash" is a claim with nothing
  ...
  if (shift) {
    await createAdminClient().from("activity_log").insert({
      category: "shift",
      description: `Clocked out — counted ${money(counted)} in the drawer`,
      actor: viewer.profile.id,
    });

    await pushToOwners({
      title: `${nameOf(viewer)} clocked out`,
      body: `${shiftLength(shift.started_at, shift.ended_at)} on shift, ${money(counted)} counted in the drawer.`,
      url: "/admin/staff",
      tag: `shift-${viewer.profile.id}`,
    });
  }
