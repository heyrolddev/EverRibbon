  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Tile
          label="Sales, 30 days"
          value={money(snapshot.revenue.last30)}
          detail={
            growth === null
        ...
        <Tile
          label="Average order"
          value={money(snapshot.revenue.avgOrder)}
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        >
          <ColumnChart data={weekday} hue="money" format="money" />
        </Panel>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        ...
        >
          {snapshot.slowMovers.length === 0 ? (
            <p className="text-sm text-ink-900/60">Nothing on the menu yet.</p>
          ) : (
            <ul className="flex flex-col gap-2">
              {snapshot.slowMovers.map((m) => (
                <li
                  key={m.name}
                  className="flex items-center justify-between gap-3 rounded-xl bg-paper-50 px-4 py-2.5 text-sm ring-1 ring-ink-950/10"
                >
                  <span className="min-w-0 truncate font-semibold text-ink-950">
                    {m.name}
                  </span>
                  <span className="shrink-0 text-ink-900/60">
                    {m.qty === 0 ? "none sold" : `${m.qty} sold`} · {money(m.price)}
                  </span>
                </li>
              ))}
            </ul>
...
function Panel({
  ...
  return (
    <div className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10">
      ...
      <p className="mb-4 mt-0.5 text-xs text-ink-900/55">{note}</p>
      {children}
    </div>
  );
}
