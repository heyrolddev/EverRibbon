...
const manilaParts = new Intl.DateTimeFormat(brand.locale, {
    weekday: "short",
  hour: "numeric",
  hour12: false,
});
...
export async function buildSnapshot(): Promise<ShopSnapshot> {
  ...
  // A product nobody ordered never appears in order_lines, so the slow movers
  ...
  for (const o of last30) {
    ...
    hourTally[b.hour] = (hourTally[b.hour] ?? 0) + 1;
