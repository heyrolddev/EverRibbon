...
export default async function AdminCounterPage() {
  ...
  const [{ data: products, error }, { data: today }, { data: catRows }] = await Promise.all([
    supabase
      .from("products")
  ...
  const rows: CounterMeal[] = ((products ?? []) as CounterMeal[])
    .map((m) => ({ ...m, makeable: makeable.get(m.id) ?? null }))
    .filter(
    // Sold out is sold out at the counter too — the whole point of 86ing
    // something is that nobody sells it. Hidden-from-the-website products stay,
