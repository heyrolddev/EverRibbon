...
export type CounterLine = { productId: string; qty: number };
...
export async function recordWalkInSale(input: {
  ...
  const priceById = new Map(
    ((products ?? []) as { id: string; price: number }[]).map((m) => [
      m.id,
      Number(m.price) || 0,
    ])
  );
  const missing = lines.filter((l) => !priceById.has(l.productId));
  if (missing.length > 0) {
    return {
  ...
  const short = lines
    .map((l) => ({ line: l, can: makeable.get(l.productId) }))
  ...
  if (short.length > 0) {
    ...
    const detail = short
      .map(
        (x) =>
          `${nameById.get(x.line.productId) ?? "an item"} (${x.can} left, ${x.line.qty} rung up)`
      )
      .join(", ");
    return {
  ...
  const subtotal = lines.reduce(
    (sum, l) => sum + priceById.get(l.productId)! * l.qty,
    0
  );
  ...
  const { error: linesError } = await supabase.from("order_lines").insert(
    lines.map((l) => ({
      ...
      qty: l.qty,
      price_at_sale: priceById.get(l.productId)!,
    }))
  );
  if (linesError) {
  ...
  const { error: logError } = await supabase.from("activity_log").insert({
    category: "orders",
    description:
      `Rang up ${label} at the counter — ${money(subtotal)} ` +
