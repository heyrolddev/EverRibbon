"use server";
import { money, shopToday } from "@/lib/format";
 ...
 *
 * Was `isStaff` — everyone who worked here — which put every material's
 ...
 * product cost by ten, so the form asks what they actually paid and for how
...
export async function saveIngredient(input: {
  ...
  if (input.id) {
    ...
    if (!data?.length) return { error: "That material no longer exists." };

    await log("inventory", `Edited material "${name}"`, viewer.profile?.id ?? null);
    revalidate();
  ...
  await log("inventory", `Added material "${name}"`, viewer.profile?.id ?? null);
  revalidate();
...
export async function deleteIngredient(id: string): Promise<Result> {
  ...
  // Refused rather than cascaded. Deleting a material a recipe still uses
  // would leave that product costing less than it does, which reads as a product
  ...
  const [{ data: inRecipes }, { data: inBatches }] = await Promise.all([
    supabase.from("product_materials").select("product_id").eq("ref_type", "inv").eq("ref_id", id).limit(5),
    supabase.from("production_run_materials").select("production_run_id").eq("material_id", id).limit(5),
  ]);
 ...
 * price has moved — updates the standard cost so every product that uses it
 * reprices.
 */
export async function recordRestock(input: {
  materialId: string;
  /** How much arrived, in the material's own unit. */
  qty: number;
  ...
  if (!ing) return { error: "That material no longer exists." };
  ...
  await log(
    "inventory",
    `Restocked ${input.qty} ${ing.unit} of "${ing.name}" for ${money(input.amountPaid)}` +
      (input.updateStandardCost && priceMoved
        ? ` — cost per ${ing.unit} now ${money(lotCost, 4)}`
        : ""),
    viewer.profile?.id ?? null
  );
  revalidate();
...
export async function adjustStock(input: {
  ...
  if (variance > 0) {
    await supabase.rpc("restore_material", {
    ...
    await supabase.rpc("consume_material", {
      p_ingredient_id: ing.id,
      p_qty: -variance,
      p_date: shopToday(),
      p_type: "count",
 ...
 * would take the materials without producing the sauce.
 *
 ...
 */
export async function produceRun(input: {
  ...
  const made = Number(production_run.yield_qty) * input.multiplier;
  await log(
    "movement",
    `Made ${input.multiplier}× "${production_run.name}" — ${made.toLocaleString(brand.locale)} ${production_run.yield_unit}, cost ${money(Number(data ?? 0))}`,
    viewer.profile?.id ?? null
  );
  revalidate();
...
export async function saveBatchRecipe(input: {
  ...
  if (lines.length > 0) {
    const { error } = await supabase.from("production_run_materials").insert(
      lines.map((l) => ({
  ...
  await log(
    "inventory",
    `Changed the recipe for "${production_run.name}" — ${lines.length} material${lines.length === 1 ? "" : "s"}`,
    viewer.profile?.id ?? null
  );
  revalidate();
  return { error: null };
...
export async function saveMealRecipe(input: {
  ...
  await log(
    "inventory",
    `Changed the recipe for "${product.name}" — ${lines.length} line${lines.length === 1 ? "" : "s"}`,
    viewer.profile?.id ?? null
  );
  revalidate();
  revalidatePath("/menu");
 ...
 * products and tasting portions — also money, but money spent deliberately.
 ...
 * right either way. It is logged to `material_usage` under its own type
...
export async function recordWaste(input: {
  ...
  if (input.sourceType === "inv") {
    ...
    const { data: cost, error } = await supabase.rpc("consume_material", {
      p_ingredient_id: ing.id,
      p_qty: input.qty,
      p_date: today,
      p_type: logType,
    ...
    await log(
      "waste",
      `${input.category === "internal" ? "Internal use" : "Waste"}: ${input.qty} ${ing.unit} of "${ing.name}" — ${money(total)} (${input.reason.trim()})`,
      viewer.profile?.id ?? null
    );
    revalidate();
  ...
  const { error: stockError } = await supabase
    .from("production_runs")
    .update({ run_stock: Number(production_run.run_stock) - input.qty })
    .eq("id", production_run.id);
  if (stockError) return { error: stockError.message };
  ...
  await supabase.from("waste").insert({
    ...
    material_id: null,
    qty: input.qty,
    unit: production_run.yield_unit,
    reason: input.reason.trim(),
    cost_at_time: unitCost,
    total_cost: total,
    category: input.category,
    source_type: "production_run",
    source_id: production_run.id,
    source_name: production_run.name,
    note: input.note?.trim() || null,
    logged_by: viewer.profile?.full_name?.trim() || viewer.email,
  });
  ...
  await log(
    "waste",
    `${input.category === "internal" ? "Internal use" : "Waste"}: ${input.qty} ${production_run.yield_unit} of "${production_run.name}" — ${money(total)} (${input.reason.trim()})`,
    viewer.profile?.id ?? null
  );
  revalidate();
  return { error: null, cost: total };
...
/**
 * What a product needs to travel, per serving.
 *
 * Stored apart from the recipe on purpose. A product eaten at the stall uses
 * none of it, and rolling packaging into the recipe is exactly what forced
 * 27 duplicate "(T.O)" products onto this menu — two entries, two prices to
...
export async function saveMealPackaging(input: {
  ...
  if (lines.length > 0) {
    const { error } = await supabase.from("product_packaging").insert(
      lines.map((l) => ({
        product_id: input.productId,
        ref_type: l.refType,
        ref_id: l.refId,
  ...
  await log(
    "inventory",
    `Changed take-out packaging for "${product.name}" — ${lines.length} item${lines.length === 1 ? "" : "s"}`,
    viewer.profile?.id ?? null
  );
  revalidate();
  return { error: null };
 ...
 *
 * Separate from per-product packaging because it does not multiply. Pricing the
 * bag into each product charges four bags for a four-product order, which is what
