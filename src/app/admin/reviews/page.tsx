...
export default async function AdminReviewsPage() {
  ...
  const { data, error } = await supabase
    .from("reviews")
    .select(
      "id, customer_id, author_name, source, relayed_by, product_id, rating, comment, shop_reply, is_hidden, created_at"
    )
  ...
  if (error) {
    return (
      <div className="rounded-3xl bg-brand-50 p-8 ring-2 ring-brand-700/40">
        <h2 className="font-display text-2xl font-black text-brand-800">
  ...
  // The product list for the relay form, not just the products already reviewed —
  ...
  const products = new Map(mealList.map((m) => [m.id, m.name]));
  ...
  const reviews: AdminReview[] = rows.map((r) => {
    ...
    return {
      ...
      mealName: r.product_id ? (products.get(r.product_id) ?? "A product") : null,
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm text-ink-900/60">
          Customers can only review a product they actually bought on a completed
      ...
      {visible.length > 0 && (
        <div className="flex flex-wrap items-center gap-6 rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
          <div>
      ...
      {canRelay && <RelayedReviewForm products={mealList} />}

      <p className="rounded-2xl bg-accent-50 px-5 py-3 text-sm text-ink-900 ring-1 ring-accent-200/40">
