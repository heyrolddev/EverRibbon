...
export const metadata: Metadata = {
  ...
  title: {
    ...
    // Inner pages get "Menu · <shop>" without each one repeating it.
...
export const viewport: Viewport = {
  // The colour a phone paints its own chrome with, per theme, so the browser
  // frame matches the page instead of sitting on a strip of the wrong white.
  themeColor: [
    { media: "(prefers-color-scheme: light)", color: brand.palette["paper-50"] },
    { media: "(prefers-color-scheme: dark)", color: brand.palette["ink-950"] },
  ],
};
...
export default async function RootLayout({ children }: LayoutProps<"/">) {
  ...
  return (
    ...
    >
      <head>
        {/*
          The shop's palette, as custom properties, ahead of everything that
          reads them. It lives in the document rather than in a CSS file
          because it is per-shop data: compiling it into the bundle is what
          would make this one shop's website instead of a system any shop can
          run. First in <head> so nothing paints before the colours exist.
        */}
