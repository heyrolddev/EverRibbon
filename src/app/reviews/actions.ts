...
/**
 * Leave or update a review. `productId` null rates the shop overall.
 *
 * Eligibility is enforced by RLS (`has_bought_product` / `has_completed_order`),
...
export async function saveReview(input: {
  ...
  // The unique indexes make this one review per customer per product, so an
  // existing one is updated rather than duplicated. `product_id IS NULL` needs
  // `.is()`, not `.eq()`, which is why the two cases are separate.
  const base = supabase.from("reviews").select("id").eq("customer_id", user.id);
  const existing = await (input.productId === null
 ...
 * misspelt name or the wrong product is a mistake to erase rather than to hide.
