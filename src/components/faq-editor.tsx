...
export type FaqRow = {
  ...
  priority: number;
  updated_at: string;
  /** Printed in the homepage's FAQ as well as given by Ask {brand.name}. */
  show_on_site: boolean;
  site_order: number;
};

const fieldClass =
  "rounded-xl border-2 border-ink-950/15 bg-paper-50 px-3 py-2 text-sm outline-none focus:border-brand-700";
const labelClass =
  "flex flex-col gap-1 text-xs font-bold uppercase tracking-widest text-ink-900";
...
export function FaqEditor({
  ...
  return (
    <div className="flex flex-col gap-4">
      {gaps.length > 0 && (
        <div className="flex flex-col gap-3 rounded-3xl bg-ink-950 p-6 text-paper-100">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-accent-200">
              People keep asking
            </p>
            <p className="mt-1 font-display text-xl font-black text-paper-50">
          ...
          <ul className="flex flex-col gap-2">
            {gaps.map((gap) => (
              <li
                key={gap.topic}
                className="flex flex-wrap items-center gap-3 rounded-2xl bg-paper-50/5 p-4 ring-1 ring-paper-50/10"
              >
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-accent-200 font-mono text-sm font-black text-ink-950">
                  {gap.count}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block font-mono text-sm font-bold text-accent-200">
                    {gap.topic}
                  </span>
                  <span className="mt-0.5 block text-sm text-paper-100/70">
                ...
                <button
                  ...
                  className="shrink-0 rounded-full bg-accent-200 px-4 py-2 text-xs font-bold text-ink-950 transition-transform hover:scale-105"
                >
                  Answer this
                </button>
              </li>
      ...
      {adding ? (
        ...
        <button
          ...
          className="self-start rounded-full bg-brand-700 px-6 py-3 font-bold text-paper-50 transition-transform hover:scale-105"
        >
          + Add an answer
        </button>
      )}
      ...
      {rows.length === 0 ? (
        ...
        >
          Ask {brand.name} still handles the menu, prices, delivery, payment,
        ...
        <ul className="flex flex-col gap-3">
          {rows.map((row) =>
            editingId === row.id ? (
              ...
              <li
                key={row.id}
                className={`rounded-3xl bg-paper-50 p-5 ring-1 ${
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 whitespace-pre-wrap text-sm text-ink-900/75">
                      {row.answer}
                    </p>
                  </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                  {row.triggers.map((t) => (
                    <span
                      key={t}
                      className="rounded-full bg-paper-200 px-2 py-0.5 font-mono text-[10px] font-bold text-ink-900"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <p className="mt-2 flex flex-wrap items-center gap-x-1 text-[11px] text-ink-900/45">
                  {row.show_on_site && row.is_active && (
                    <span className="mr-1 rounded-full bg-accent-200 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-ink-950">
                      On the homepage
                    </span>
                  )}
                  <span>
...
function NewAnswer({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-3xl bg-accent-50 p-5 ring-1 ring-accent-200/50">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="rounded-full bg-brand-700 px-5 py-2.5 text-sm font-bold text-paper-50 disabled:opacity-50"
        >
          {busy ? "Saving…" : "Save answer"}
        </button>
        <button
          onClick={onDone}
          className="rounded-full px-4 py-2.5 text-sm font-bold text-ink-900 hover:text-brand-700"
        >
          Cancel
        </button>
      </div>
...
function EditAnswer({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-3xl bg-paper-100 p-5 ring-2 ring-brand-700/30">
      <label className={labelClass}>
        The question
        <input
          value={question}
      ...
      <div className="flex flex-wrap items-center gap-5">
        ...
        <label className="flex items-center gap-2 text-sm font-semibold text-ink-900">
          ...
          <input
            ...
            className="w-20 rounded-xl border-2 border-ink-950/15 bg-paper-50 px-2 py-1 text-sm outline-none focus:border-brand-700"
          />
          <span className="text-[11px] font-medium text-ink-900/50">
            wins ties
          </span>
        </label>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-5 rounded-2xl bg-paper-50 px-4 py-3 ring-1 ring-ink-950/10">
        <label className="flex items-center gap-2 text-sm font-semibold text-ink-900">
          <input
            type="checkbox"
            checked={showOnSite}
            onChange={(e) => setShowOnSite(e.target.checked)}
            className="h-4 w-4 accent-accent-200"
          />
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {confirmDelete ? (
          <button
            onClick={remove}
            disabled={busy}
            className="ml-auto rounded-full bg-brand-700 px-4 py-2.5 text-sm font-bold text-paper-50"
          >
            Really delete?
          </button>
        ) : (
          <button
            ...
            className="ml-auto rounded-full px-4 py-2.5 text-sm font-bold text-brand-800 hover:underline"
          >
            Delete
          </button>
        )}
