...
export function ReorderButton({ orderId }: { orderId: string }) {
  ...
  async function run() {
    ...
    // about to look for the missing product.
    const note =
  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        className="rounded-full bg-brand-700 px-5 py-2 text-sm font-bold text-paper-50 transition-transform hover:scale-105 disabled:opacity-60"
      >
        {busy ? "Adding…" : "Order this again →"}
      </button>
      {error && (
        <p className="text-sm font-semibold text-brand-800">{error}</p>
      )}
    </div>
  );
}
