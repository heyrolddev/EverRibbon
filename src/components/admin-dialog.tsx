...
export function AdminDialog({
  ...
  return createPortal(
    ...
    >
      ...
      <div className="relative max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-paper-50 p-6 shadow-2xl ring-1 ring-ink-950/10 sm:max-w-lg sm:rounded-3xl">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h2 className="font-display text-2xl font-black text-ink-950">{title}</h2>
            {subtitle && (
              <p className="mt-1 text-sm text-ink-900/60">{subtitle}</p>
            )}
          </div>
          <button
            onClick={() => !busy && onClose()}
            aria-label="Close"
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-ink-950/5 text-ink-900 transition-colors hover:bg-ink-950/10"
          >
            ✕
          </button>
        </div>
...
export function Field({
  ...
  return (
    <label className="flex flex-col gap-1.5">
      <span className="text-[11px] font-black uppercase tracking-widest text-ink-900/60">
        {label}
      </span>
      {children}
...
export const inputClass =
  "w-full rounded-xl border-2 border-ink-950/10 bg-paper-100 px-4 py-2.5 text-ink-950 outline-none transition-colors placeholder:text-ink-900/35 focus:border-accent-200";
