...
export function CounterTill({
  ...
  const byId = useMemo(() => new Map(products.map((m) => [m.id, m])), [products]);
  ...
  const shown = useMemo(() => {
    ...
    return sorted.filter((m) => {
      // ANY of the product's categories. Reading only the first meant a product
  ...
  return (
    <div className="flex flex-col gap-6 pb-24 lg:pb-0">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="rounded-2xl bg-paper-100 px-4 py-2.5 text-right ring-1 ring-ink-950/10">
          <p className="text-[10px] font-black uppercase tracking-widest text-ink-900/40">
            Counter sales today
          </p>
          <p className="font-display text-xl font-black tabular-nums text-ink-950">
            {money(takenToday, 0)}
            <span className="ml-2 text-xs font-bold text-ink-900/40">
      ...
      {done && (
        <div className="overflow-hidden rounded-2xl ring-1 ring-ink-950/10">
          <div className="flex flex-wrap items-center justify-between gap-3 bg-ok-600 px-5 py-4 text-paper-50">
            <p className="font-display text-lg font-black">
              Recorded {money(done.total, 0)} {done.dineIn ? "dine in" : "take out"}
          ...
          {printed && (
            <p className="bg-ok-700 px-5 py-2 text-sm font-semibold text-paper-50">
              {printed}
            </p>
          )}
          ...
          {/* The receipt is offered, never forced. The sale is already
              ...
              transaction. */}
          <details className="bg-paper-100">
            <summary className="cursor-pointer list-none px-5 py-3 text-sm font-bold text-ink-900/70 hover:text-ink-950">
              Receipt &amp; printing ▾
            </summary>
      ...
      {review && (
        ...
        >
          <div
            ...
            className="flex max-h-[92vh] w-full max-w-md flex-col overflow-hidden rounded-t-3xl bg-paper-50 shadow-2xl sm:rounded-3xl"
          >
            <div className="flex items-baseline justify-between gap-3 bg-ink-950 px-5 py-4">
              <p className="font-display text-lg font-black text-paper-50">
                Check this order
              </p>
              <p className="font-display text-lg font-black text-accent-200">
                {money(review.total, 0)}
              </p>
            </div>

            <div className="overflow-y-auto px-5 py-4">
              <pre className="overflow-x-auto rounded-2xl bg-paper-100 px-4 py-4 font-mono text-[12px] leading-[1.45] text-ink-950">
                {asPlainText(renderReceipt(review)).join("\n")}
              </pre>
              <p className="mt-2 text-[11px] text-ink-900/50">
            ...
            {error && (
              <p className="mx-5 mb-3 rounded-xl bg-brand-700 px-3 py-2 text-sm font-semibold text-paper-50">
                {error}
              </p>
            )}

            <div className="flex gap-3 border-t border-ink-950/10 bg-paper-100 px-5 py-4">
              <button
                onClick={() => setReview(null)}
                disabled={pending}
                className="flex-1 rounded-2xl bg-ink-950/5 py-3.5 font-bold text-ink-900 transition-colors hover:bg-ink-950/10 disabled:opacity-40"
              >
                Go back
              </button>
              <button
                onClick={submit}
                disabled={pending}
                className="flex-[1.4] rounded-2xl bg-ok-600 py-3.5 font-display text-lg font-black text-paper-50 transition-colors hover:bg-ok-700 disabled:bg-ink-950/15 disabled:text-ink-900/40"
              >
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="flex min-w-0 flex-col gap-3">
          <input
            ...
            className="rounded-xl bg-paper-100 px-4 py-3 text-base text-ink-950 ring-1 ring-ink-950/10 placeholder:text-ink-900/40 focus:outline-none focus:ring-2 focus:ring-accent-200"
          />
          ...
          {shown.length === 0 ? (
            <p className="rounded-2xl border-2 border-dashed border-brand-300 bg-paper-100 p-6 text-sm text-ink-900/70">
              Nothing here. {products.length === 0 && "Every product is marked sold out."}
            </p>
          ) : (
                  ...
                  <li key={m.id}>
                    <button
                      ...
                      className={`relative flex h-full w-full flex-col justify-between gap-2 rounded-2xl p-3 text-left transition-colors ${
                        qty > 0
                          ? "bg-ink-950 text-paper-50"
                          : "bg-paper-100 text-ink-950 ring-1 ring-ink-950/10 hover:bg-paper-200"
                      }`}
                    >
                      ...
                      {qty > 0 && (
                        <span className="absolute -right-1.5 -top-1.5 grid h-7 min-w-7 place-items-center rounded-full bg-accent-200 px-1.5 font-display text-sm font-black tabular-nums text-ink-950 ring-2 ring-paper-50">
                          {qty}
                        </span>
                      )}
                      {out ? (
                        <span className="absolute bottom-1.5 right-2 rounded-full bg-brand-700 px-1.5 text-[9px] font-black uppercase tracking-wide text-paper-50">
                          no stock
                        </span>
                      ) : (
                        !m.is_public && (
        ...
        <div id="counter-ticket" className="scroll-mt-4 lg:sticky lg:top-6 lg:self-start">
          <div className="flex flex-col rounded-3xl bg-paper-100 ring-1 ring-ink-950/10">
            ...
            {lines.length === 0 ? (
              <p className="px-5 py-8 text-center text-sm text-ink-900/40">
                ...
                {lines.map((l) => (
                  <li key={l.product.id} className="flex items-center gap-3 px-4 py-2.5">
                    {/* Wrapped, never truncated. Half this menu is
                        ...
                        at "Burger Jipai w/ch…" is two different products that
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs tabular-nums text-ink-900/50">
                        {money(l.product.price, 0)} each
                      </p>
                    </div>
            ...
            <div className="flex flex-col gap-3 border-t border-ink-950/10 p-4">
              <div className="flex items-baseline justify-between">
                <span className="text-sm font-bold text-ink-900/60">Total</span>
              ...
              <div>
                <p className="mb-1.5 text-[10px] font-black uppercase tracking-widest text-ink-900/40">
                  Serving
                </p>
                  ...
                  {([false, true] as const).map((d) => (
                    <button
                      ...
                      className={`rounded-xl py-2.5 text-sm font-black uppercase tracking-wide transition-colors ${
                        dineIn !== d
                          ? "bg-ink-950/5 text-ink-900/50 hover:bg-ink-950/10"
                          : d
                ...
                <p className="mt-1.5 text-[11px] leading-snug text-ink-900/40">
                  {dineIn
              ...
              {method === "gcash" && (
                <input
                  ...
                  className="rounded-xl bg-paper-50 px-3 py-2.5 text-sm ring-1 ring-ink-950/10 focus:outline-none focus:ring-2 focus:ring-accent-200"
                />
              )}
              ...
              {method === "cash" && lines.length > 0 && (
                <div className="rounded-xl bg-ink-950/[0.03] p-3">
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-lg px-3 py-2 text-sm font-black tabular-nums transition-colors ${
                            on
                              ? "bg-ink-950 text-paper-50"
                              : "bg-paper-50 text-ink-950 ring-1 ring-ink-950/10 hover:bg-paper-200"
                          }`}
                        >
                          {i === 0 ? "Exact" : money(amount, 0)}
                        </button>
                      );
                    })}
                  </div>
                  <input
                    ...
                    className="mt-2 w-full rounded-xl bg-paper-50 px-3 py-2.5 text-right font-display text-lg font-black tabular-nums ring-1 ring-ink-950/10 focus:outline-none focus:ring-2 focus:ring-accent-200"
                  />
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="min-w-0 flex-1 rounded-xl bg-paper-50 px-3 py-2.5 text-sm ring-1 ring-ink-950/10 focus:outline-none focus:ring-2 focus:ring-accent-200"
                />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className={`shrink-0 rounded-xl px-3 py-2.5 text-xs font-black uppercase tracking-wide transition-colors ${
                    noName
                      ? "bg-ink-950 text-paper-50"
                      : "bg-ink-950/[0.06] text-ink-900/60 hover:bg-ink-950/10"
                  }`}
                >
                  No name
                </button>
              ...
              {error && (
                <p className="rounded-xl bg-brand-700 px-3 py-2 text-sm font-semibold text-paper-50">
                  {error}
                </p>
              )}
              ...
              <button
                ...
                className="rounded-2xl bg-ok-600 py-4 font-display text-lg font-black text-paper-50 transition-colors hover:bg-ok-700 disabled:cursor-not-allowed disabled:bg-ink-950/15 disabled:text-ink-900/40"
              >
                {pending
                  ? "Recording…"
                  : lines.length === 0
      ...
      {count > 0 && (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-ink-950/10 bg-paper-50/95 px-4 py-3 backdrop-blur lg:hidden">
          <a
            href="#counter-ticket"
            className="flex items-center gap-3 rounded-2xl bg-ok-600 px-4 py-3 text-paper-50"
          >
            <span className="grid h-8 min-w-8 shrink-0 place-items-center rounded-full bg-paper-50/20 px-2 font-display text-sm font-black tabular-nums">
              {count}
            </span>
...
function Change({ total, tendered }: { total: number; tendered: string }) {
  ...
  if (state.kind === "short") {
    return (
      <p className="mt-2 flex items-baseline justify-between rounded-lg bg-brand-700 px-3 py-2 text-paper-50">
  ...
  if (state.kind === "exact") {
    return (
      <p className="mt-2 rounded-lg bg-ok-600 px-3 py-2 text-center text-sm font-black uppercase tracking-wide text-paper-50">
        Exact — no change
      </p>
    );
  }
  ...
  return (
    <p className="mt-2 flex items-baseline justify-between rounded-lg bg-ok-600 px-3 py-2 text-paper-50">
