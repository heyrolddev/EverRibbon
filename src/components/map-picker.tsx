  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const pinIcon = L.divIcon({
        className: "",
        html: `<span style="display:block;width:22px;height:22px;border-radius:9999px;background:var(--brand-700);border:3px solid var(--paper-50);box-shadow:0 2px 8px rgba(0,0,0,.4)"></span>`,
        iconSize: [22, 22],
        iconAnchor: [11, 11],
      });
      const shopIcon = L.divIcon({
        className: "",
        html: `<span style="display:block;width:18px;height:18px;border-radius:9999px;background:var(--accent-200);border:3px solid var(--ink-950);box-shadow:0 2px 8px rgba(0,0,0,.4)"></span>`,
        iconSize: [18, 18],
        iconAnchor: [9, 9],
      });
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          disabled={locating}
          className="rounded-full bg-ink-950 px-4 py-2 text-xs font-bold text-paper-50 transition-colors hover:bg-brand-700 disabled:opacity-60"
        >
      ...
      {note && <p className="text-xs font-semibold text-brand-800">{note}</p>}
    </div>
  );
}
