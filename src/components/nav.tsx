...
function ActiveRule() {
  return (
    <motion.span
      layoutId="nav-active"
      className="absolute inset-x-3 -bottom-0.5 h-0.5 rounded-full bg-accent-200"
    />
  );
}
...
export function Nav({
  ...
  const linkClass = solid
    ...
    : "text-paper-100/80 hover:text-accent-200";
  ...
  return (
    <header
      ...
      className={`sticky top-0 z-40 transition-colors duration-300 ${
        solid
          ? "bg-paper-50 shadow-[inset_0_-1px_0_rgb(28_17_14/0.12)]"
          : "bg-transparent"
      }`}
    >
      <div className="mx-auto flex h-[var(--nav-h)] max-w-6xl items-center justify-between px-6">
        ...
        <nav className="flex min-w-0 items-center gap-1.5 text-sm font-semibold sm:gap-2">
          ...
          {staff && (
            <Link
              href="/admin"
              title={`You're signed in as shop staff — open ${brand.name} HQ`}
              className={`group flex items-center gap-1.5 rounded-full py-1.5 pl-2 pr-3 font-bold ring-2 transition-all hover:scale-105 ${
                solid
                  ? "bg-ink-950 text-accent-200 ring-accent-200/40"
                  : "bg-brand-700 text-paper-50 ring-accent-200/60"
              }`}
            >
              <span
                className={`grid h-6 w-6 place-items-center rounded-full ${
                  solid ? "bg-brand-700 text-paper-50" : "bg-ink-950 text-accent-200"
                }`}
              >
            ...
            >
              ...
              {activeOrders > 0 && (
                <motion.span
                  ...
                  className={`relative ${countClass("bg-ok-600")}`}
                >
                  ...
                  <span
                    aria-hidden
                    className="absolute inset-0 animate-ping rounded-full bg-ok-600 opacity-60"
                  />
          ...
          {signedIn && !staff && activeOrders > 0 && (
            <Link
              href="/orders"
              title={`${activeOrders} order${activeOrders === 1 ? "" : "s"} in progress`}
              className="flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full bg-ok-600 px-2.5 py-1.5 text-xs font-bold text-paper-50 min-[880px]:hidden"
            >
          ...
          >
            ...
            {count > 0 && (
              <motion.span
                ...
                animate={{ scale: 1 }}
                transition={{ type: "spring", stiffness: 500, damping: 15 }}
                className={countClass("bg-brand-700")}
              >
                {countLabel(count)}
              </motion.span>
            )}
          ...
          {signedIn && !staff && (
            <Link
              ...
              className={`flex shrink-0 items-center gap-2 rounded-full py-1 pl-1 pr-1 font-bold transition-all hover:scale-105 min-[880px]:pr-3 ${
                onAccount
                  ? "bg-brand-700 text-paper-50 ring-2 ring-accent-200"
                  : solid
                    ? "bg-ink-950/5 text-ink-950 ring-1 ring-ink-950/10"
                    : "bg-paper-50/10 text-paper-50 ring-1 ring-paper-50/20"
              }`}
            >
              <span
                className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-black ${
                  onAccount ? "bg-ink-950 text-accent-200" : "bg-brand-700 text-paper-50"
                }`}
              >
