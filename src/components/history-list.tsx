    ...
    <div className={className}>
      ...
      {(items.length > initial || ranged) && (
        <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
          <label className="flex items-center gap-2 font-bold text-ink-900/60">
            From
            <input
              type="date"
              value={from}
              ...
              className="rounded-xl border-2 border-ink-950/10 bg-paper-100 px-3 py-1.5 font-semibold text-ink-950 outline-none focus:border-accent-200"
            />
          </label>
          <label className="flex items-center gap-2 font-bold text-ink-900/60">
            to
          ...
          {ranged && (
            <>
              <button
                ...
                className="rounded-full bg-ink-950/5 px-3 py-1.5 font-bold text-ink-900/70 transition-colors hover:bg-brand-700 hover:text-paper-50"
              >
                Clear dates
              </button>
              <span className="font-semibold text-ink-900/50">
      ...
      {shown.length === 0 ? (
        <p className="rounded-2xl border-2 border-dashed border-ink-950/15 bg-paper-100 p-5 text-sm text-ink-900/60">
      ...
      {!ranged && hidden > 0 && (
        <button
          ...
          className="mt-3 rounded-full bg-paper-100 px-4 py-2 text-xs font-bold text-ink-900/75 ring-1 ring-ink-950/10 transition-colors hover:bg-ink-950 hover:text-paper-50"
        >
      ...
      {!ranged && showAll && matching.length > initial && (
        <button
          ...
          className="mt-3 rounded-full px-4 py-2 text-xs font-bold text-ink-900/60 transition-colors hover:text-ink-950"
        >
          Show fewer
        </button>
      )}
