...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-3xl bg-paper-100 px-6 py-12 text-center ring-1 ring-ink-950/10">
      <span className="h-20 w-20 text-brand-700/45">{art}</span>
      <p className="font-display text-xl font-black text-ink-950">{title}</p>
      {children && (
        <p className="max-w-sm text-sm text-ink-900/60">{children}</p>
      )}
      {action}
    </div>
  );
