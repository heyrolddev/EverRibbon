...
const RING: Record<Tone, string> = {
  ...
  alert: "ring-2 ring-brand-700",
  good: "ring-2 ring-ok-600",
};
...
function useCountUp(value: string, still: boolean): string {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (t < 1) {
        // Substituted back into the rendered string, so a formatted total keeps its
        ...
        setAnim(value.replace(FIRST_NUMBER, () => Math.round(at).toLocaleString(brand.locale)));
...
export function StatTile({
  ...
  return (
    <div
      className={`min-w-0 rounded-2xl bg-paper-100 p-5 transition-shadow hover:shadow-[0_2px_16px_-4px] hover:shadow-ink-950/15 ${RING[tone]}`}
    >
      ...
      {/*
        ...
        The figure itself: a clamp alone can't know that a seven-figure total is
        nearly twice the width of a four-figure one, so a size that fits takings
      ...
      */}
      ...
      {detail && <p className="mt-1 text-sm text-ink-900/60">{detail}</p>}
      {children}
    </div>
  );
}
...
export function Delta({
  ...
  const up = pct > 0;
  return (
    <p className={`mt-1 text-sm font-bold ${up ? "text-ok-700" : "text-brand-800"}`}>
