  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <pre
        ref={paper}
        id="receipt-paper"
        className="receipt-paper overflow-x-auto rounded-2xl bg-paper-50 px-4 py-5 font-mono text-[12.5px] leading-[1.45] text-ink-950 ring-1 ring-ink-950/15"
      >
        {text.join("\n")}
      </pre>
      ...
      <div className="flex flex-wrap gap-2">
        {canBluetooth && (
          <button
            ...
            disabled={busy}
            className="rounded-2xl bg-ink-950 px-5 py-3 font-bold text-accent-200 transition-transform hover:scale-105 disabled:opacity-50"
          >
            {busy
              ? "Printing…"
              : connected
        ...
        <button
          ...
          className="rounded-2xl bg-ink-950/5 px-5 py-3 font-bold text-ink-900 transition-colors hover:bg-ink-950/10"
        >
          Print dialog
        </button>
        ...
        {connected && (
          <button
            ...
            className="rounded-2xl px-4 py-3 text-sm font-bold text-ink-900/60 hover:text-brand-700"
          >
            Disconnect
          </button>
        )}
      ...
      {connected && (
        <label className="flex cursor-pointer items-center gap-3 rounded-2xl bg-paper-100 px-4 py-3">
          <input
            type="checkbox"
            checked={auto}
          ...
          <span className="text-sm font-semibold text-ink-950">
            ...
            <span className="mt-0.5 block text-xs font-medium text-ink-900/60">
      ...
      {note && <p className="text-sm font-semibold text-ok-700">{note}</p>}
      {error && (
