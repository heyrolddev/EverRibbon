...
export function DateRangePicker({
  ...
  return (
    <div className="flex flex-wrap items-end gap-3 rounded-2xl bg-paper-100 p-4 ring-1 ring-ink-950/10">
      <label className="flex flex-col gap-1 text-[11px] font-bold uppercase tracking-widest text-ink-900/55">
        From
        <input
          type="date"
          value={start}
          ...
          className="rounded-xl border-2 border-ink-950/15 bg-paper-50 px-3 py-2 text-sm font-semibold normal-case tracking-normal text-ink-950 outline-none focus:border-brand-700"
        />
      </label>
      ...
      <div className="flex flex-wrap gap-1.5">
        {presets.map((p) => (
          <button
            ...
            className="rounded-full bg-paper-50 px-3 py-1.5 text-xs font-bold text-ink-900 ring-1 ring-ink-950/10 transition-colors hover:bg-accent-200 hover:text-ink-950"
          >
            {p.label}
          </button>
        ))}
