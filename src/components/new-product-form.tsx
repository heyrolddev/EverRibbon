...
export function NewMealForm({ categories }: { categories: MenuCategory[] }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="rounded-full bg-ink-950 px-6 py-3 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-700"
      >
        + Add menu item
      </button>
    );
  ...
  return (
    <form
      ...
      className="flex flex-col gap-3 rounded-2xl bg-paper-100 p-5 ring-2 ring-brand-700"
    >
