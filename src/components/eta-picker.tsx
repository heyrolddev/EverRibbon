...
export function EtaPicker({
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      {open && (
        <div className="flex flex-wrap justify-end gap-1.5 rounded-2xl bg-paper-50 p-2 ring-1 ring-ink-950/10">
          {PRESETS.map((m) => (
            <button
              key={m}
              onClick={() => set(m)}
              disabled={pending}
              className="rounded-full bg-ink-950/10 px-3 py-1 text-xs font-bold text-ink-950 transition-colors hover:bg-brand-700 hover:text-paper-50 disabled:opacity-60"
            >
              {m}m
            </button>
          ))}
          {eta != null && (
            <button
              onClick={() => set(null)}
              disabled={pending}
              className="rounded-full px-3 py-1 text-xs font-bold text-ink-900 hover:text-brand-700 disabled:opacity-60"
            >
              Clear
            </button>
          )}
