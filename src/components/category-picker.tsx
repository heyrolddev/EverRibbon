 ...
 *
 * This was a plain text box on every product form, which is how a menu ends up
 ...
 * a product. It's just no longer the default path.
 *
 * More than one, because a product is genuinely more than one thing. Chicken
 * Chop is chicken AND it is rice products AND it is what the shop is known for;
 * forcing a choice between those meant the product only appeared under whichever
...
export function CategoryPicker({
  ...
  /** Every category on this product. Order is kept — the first one leads. */
  value: string[];
  ...
  const add = (name: string) => {
    ...
    // one product is exactly the mess this picker exists to prevent.
        ...
        <div className="mb-2 flex flex-wrap gap-1.5">
          {value.map((name, i) => {
            ...
            return (
              <span
                key={name}
                className="inline-flex items-center gap-1.5 rounded-full bg-paper-100 py-1 pl-2 pr-1 text-xs font-bold text-ink-950 ring-1 ring-ink-950/10"
              >
                <span aria-hidden className={`h-2.5 w-2.5 rounded-full ${tone.dot}`} />
                {name}
                {/* The first one leads — it is what the product reads as on a
                ...
                <button
                  ...
                  className="grid h-5 w-5 place-items-center rounded-full text-ink-900/40 transition-colors hover:bg-brand-700 hover:text-paper-50"
                >
                  ×
                </button>
              </span>
      ...
      {showNew || categories.length === 0 ? (
        <div className="flex items-center gap-2">
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                // The form would otherwise submit on Enter, saving the product
            ...
            autoFocus={showNew}
            className="min-w-0 flex-1 rounded-xl border-2 border-ink-950/10 bg-paper-100 px-4 py-2.5 text-ink-950 outline-none transition-colors focus:border-accent-200"
          />
          <button
            type="button"
            onClick={() => add(draft)}
            disabled={!draft.trim()}
            className="shrink-0 rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-700 disabled:opacity-40"
          >
            Add
          </button>
        </div>
