...
export function SafetyNetList({ snapshots }: { snapshots: SnapshotRow[] }) {
  return (
    <section className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10 sm:p-8">
      ...
      <p className="mt-2 max-w-2xl text-sm text-ink-900/55">
      ...
      {snapshots.length === 0 ? (
        <p className="mt-5 rounded-2xl border-2 border-dashed border-ink-950/15 bg-paper-50 p-5 text-sm text-ink-900/60">
        ...
        <ul className="mt-5 flex flex-col gap-2">
          {snapshots.map((s) => (
            <li
              key={s.id}
              className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-paper-50 px-4 py-3 ring-1 ring-ink-950/10"
            >
                ...
                <p className="flex flex-wrap items-center gap-2 text-sm font-bold text-ink-950">
                  ...
                  <span
                    className={`rounded-md px-1.5 py-0.5 text-[10px] font-black uppercase tracking-wide ${
                      s.automatic
                        ? "bg-ok-600/15 text-ok-700"
                        : "bg-accent-200/25 text-ink-900/70"
                    }`}
                  >
              ...
              <a
                ...
                className="shrink-0 rounded-full bg-ink-950 px-4 py-2 text-xs font-bold text-paper-50 transition-colors hover:bg-brand-700"
              >
                Download
              </a>
            </li>
