...
const REASONS: Record<WasteCategory, string[]> = {
  ...
  internal: ["Staff product", "Tasting", "Sample for a customer", "Photo shoot"],
};

/**
 * Something didn't get sold.
 *
 * The category toggle is the whole design. Spoilage and staff products both cost
    ...
    >
      <form onSubmit={submit} className="flex flex-col gap-4">
        ...
        <Field label="What was it">
          <Combobox
            ...
            options={options.map((o) => ({
              value: `${o.kind}:${o.id}`,
              label: o.kind === "production_run" ? `${o.name} (production_run)` : o.name,
              hint: `${o.stock.toLocaleString(brand.locale)} ${o.unit}`,
            }))}
            onChange={setPick}
          />
        </Field>
        ...
        <Field
          ...
          hint={
            chosen
              ? `${chosen.stock.toLocaleString(brand.locale)} ${chosen.unit} on hand.`
              : undefined
          }
        >
          <input
        ...
        {short && (
          <p className="rounded-xl bg-accent-200 px-4 py-3 text-sm text-ink-950">
            That&apos;s more than the {chosen!.stock.toLocaleString(brand.locale)}{" "}
