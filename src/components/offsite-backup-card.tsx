...
export function OffsiteBackupCard({ settings }: { settings: OffsiteSettings }) {
  ...
  return (
    <section className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10 sm:p-8">
      ...
      {!settings.configured ? (
        <p className="mt-5 rounded-2xl border-2 border-dashed border-ink-950/15 bg-paper-50 p-5 text-sm text-ink-900/65">
          Email isn&apos;t set up on this site yet, so there is nothing to send
          with. It needs <code className="rounded bg-paper-200 px-1">RESEND_API_KEY</code>{" "}
          and <code className="rounded bg-paper-200 px-1">SHOP_FROM_EMAIL</code> —
          the same pair that sends customers their order updates.
        </p>
      ) : (
        <>
          <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-2xl bg-paper-50 px-4 py-3.5 ring-1 ring-ink-950/10">
            <input
              type="checkbox"
              checked={enabled}
              onChange={(e) => setEnabled(e.target.checked)}
              className="mt-0.5 h-5 w-5 shrink-0 accent-ok-600"
            />
            <span className="text-sm text-ink-900/75">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <input
              ...
              className="min-w-0 flex-1 rounded-xl bg-paper-50 px-3 py-2.5 text-sm ring-1 ring-ink-950/10 focus:outline-none focus:ring-2 focus:ring-accent-200 sm:max-w-sm"
            />
            <button
              onClick={save}
              disabled={busy || !dirty}
              className="rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-black text-paper-50 transition-colors hover:bg-brand-700 disabled:opacity-40"
            >
              {busy ? "Saving…" : "Save"}
            </button>
            {settings.enabled && (
              <button
                onClick={sendNow}
                disabled={busy}
                className="rounded-xl bg-paper-50 px-4 py-2.5 text-sm font-bold text-ink-950 ring-1 ring-ink-950/15 transition-colors hover:bg-paper-200 disabled:opacity-40"
              >
          ...
          {saved && (
            <p className="mt-3 rounded-2xl bg-ok-600/15 px-4 py-3 text-sm font-semibold text-ok-700">
              {saved}
            </p>
          )}
