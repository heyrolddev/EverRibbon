...
export function LiveOrdersBanner() {
  ...
  const { connected } = useOrderRealtime({
    ...
    onInsert: (row) => {
      ...
      const message = `${name} just ordered ${money(total)}`;
      ...
      if (typeof Notification !== "undefined" && Notification.permission === "granted") {
        new Notification("New ${brand.name} order", { body: message });
      }
    },
  });
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          <LiveDotIcon
            className={`h-2 w-2 ${connected ? "animate-pulse text-paper-50" : "text-ink-900/50"}`}
          />
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-2xl bg-ink-950 px-5 py-4 text-paper-50 shadow-2xl shadow-ink-950/40"
          >
            <span className="text-2xl">🔔</span>
            <div>
              <p className="font-display font-bold">New order</p>
              <p className="text-sm text-paper-100/75">{toast}</p>
            </div>
            <button
              onClick={() => setToast(null)}
              aria-label="Dismiss"
              className="ml-2 rounded-full px-2 text-paper-100/50 transition-colors hover:text-paper-50"
            >
              ✕
            </button>
          </motion.div>
