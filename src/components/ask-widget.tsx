  ...
  return (
    <>
      ...
      <div className="group fixed bottom-24 right-4 z-40 sm:bottom-6">
        <span
          aria-hidden
          className="pointer-events-none absolute inset-0 rounded-full bg-accent-200/40 motion-safe:animate-ping"
        />
        <span
          aria-hidden
          className="pointer-events-none absolute -inset-1 rounded-full ring-2 ring-accent-200/30"
        />
        <button
          onClick={() => setOpen((v) => !v)}
          aria-label="Ask ${brand.name}"
          className="relative flex h-14 w-14 items-center justify-center rounded-full bg-ink-950 text-paper-50 shadow-2xl shadow-ink-950/40 ring-2 ring-accent-200/60 transition-transform hover:scale-105"
        >
        ...
        <span
          aria-hidden
          className="pointer-events-none absolute right-full top-1/2 mr-3 hidden -translate-y-1/2 whitespace-nowrap rounded-full bg-ink-950 px-3 py-1.5 text-sm font-bold text-paper-50 opacity-0 shadow-lg transition-opacity group-hover:opacity-100 sm:block"
        >
          Ask ${brand.name}
        </span>
      </div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-x-3 bottom-3 z-50 flex max-h-[80vh] flex-col overflow-hidden rounded-3xl bg-paper-50 shadow-2xl shadow-ink-950/50 ring-1 ring-ink-950/15 sm:inset-x-auto sm:right-6 sm:bottom-6 sm:w-96"
          >
            {/* Header */}
            <div className="flex items-center justify-between gap-3 bg-ink-950 px-5 py-4 text-paper-50">
              <div>
                <p className="font-display font-black">Ask {brand.name}</p>
                <p className="text-[11px] text-paper-100/60">
              ...
              <button
                ...
                className="rounded-full px-2 text-paper-100/60 transition-colors hover:text-paper-50"
              >
                ✕
              </button>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
              {msgs.length === 0 && (
                <div className="flex flex-col gap-3">
                  <p className="rounded-2xl bg-paper-100 px-4 py-3 text-sm text-ink-900">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {OPENERS.map((o) => (
                      <button
                        key={o}
                        onClick={() => send(o)}
                        className="rounded-full bg-paper-100 px-3 py-1.5 text-xs font-semibold text-ink-900 ring-1 ring-ink-950/10 transition-colors hover:bg-brand-700 hover:text-paper-50"
                      >
                        {o}
                      </button>
                    ))}
              ...
              <ul className="flex flex-col gap-3">
                {msgs.map((m, i) => (
                  <li key={m.id ?? `local-${i}`} className="flex flex-col">
                    {m.role === "staff" && (
                      <span className="mb-1 self-start rounded-full bg-accent-200 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-ink-950">
                        ${brand.name} · owner
                      </span>
                    )}
                    <span
                      className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-sm ${
                        m.role === "user"
                          ? "self-end bg-brand-700 text-paper-50"
                          : m.role === "staff"
                            ? "self-start bg-ink-950 text-paper-100 ring-2 ring-accent-200/50"
                ...
                {takenOver && !busy && (
                  <li className="self-center rounded-full bg-accent-50 px-3 py-1 text-[11px] font-semibold text-ink-900/70 ring-1 ring-accent-200/40">
                    You&apos;re talking to the owner now
                  </li>
                )}
                {busy && (
                  <li className="self-start rounded-2xl bg-paper-100 px-4 py-2.5 text-sm text-ink-900/60">
              ...
              {needsHuman && !contactSent && (
                <div className="mt-4 flex flex-col gap-2 rounded-2xl bg-accent-50 p-4 ring-1 ring-accent-200/50">
                  ...
                  <input
                    ...
                    className="rounded-xl border-2 border-ink-950/15 bg-paper-50 px-3 py-2 text-sm outline-none focus:border-brand-700"
                  />
                  <input
                    value={phone}
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {messengerUrl && (
                      <a
                        ...
                        className="rounded-full bg-ink-950 px-4 py-2 text-xs font-bold text-paper-50"
                      >
                        Chat on Messenger ↗
                      </a>
                    )}
              ...
              {contactSent && (
                <p className="mt-4 rounded-2xl bg-ok-50 px-4 py-3 text-sm font-semibold text-ok-700 ring-1 ring-ok-600/40">
                  ✓ Sent! The owner will message you shortly.
                </p>
              )}

              {error && (
                <p className="mt-3 rounded-xl bg-brand-50 px-4 py-2 text-xs font-semibold text-brand-800">
                  {error}
                </p>
              )}
            </div>
            ...
            <form
              ...
              className="flex items-center gap-2 border-t border-ink-950/10 bg-paper-100 p-3"
            >
              <input
                value={draft}
                ...
                maxLength={1000}
                className="flex-1 rounded-full border-2 border-ink-950/15 bg-paper-50 px-4 py-2.5 text-sm outline-none focus:border-brand-700"
              />
              <button
                type="submit"
                disabled={busy || !draft.trim()}
                className="shrink-0 rounded-full bg-brand-700 px-4 py-2.5 text-sm font-bold text-paper-50 disabled:opacity-50"
              >
                Send
              </button>
            </form>

            <p className="bg-paper-100 px-4 pb-3 text-center text-[10px] text-ink-900/45">
