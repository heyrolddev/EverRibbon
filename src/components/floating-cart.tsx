        ...
        {show && (
          ...
          >
            <button
              ...
              className="mx-auto flex w-full max-w-lg items-center gap-4 rounded-full bg-brand-700 py-3 pl-5 pr-3 text-left shadow-2xl shadow-ink-950/40 ring-2 ring-accent-200/60 transition-transform hover:scale-[1.02]"
            >
              <span className="relative grid h-11 w-11 shrink-0 place-items-center rounded-full bg-ink-950">
                ...
                <motion.span
                  ...
                  className="absolute -right-1 -top-1 grid h-5 w-5 place-items-center rounded-full bg-accent-200 text-[11px] font-black text-ink-950"
                >
                  {count}
                </motion.span>
              </span>

              <span className="min-w-0 flex-1 text-paper-50">
                <span className="block text-[11px] font-bold uppercase tracking-widest text-paper-100/70">
              ...
              <span className="shrink-0 rounded-full bg-accent-200 px-5 py-3 font-display font-black text-ink-950">
                Review
              </span>
            </button>
          </motion.div>
          ...
          <div className="flex flex-col gap-5">
            <ul className="flex flex-col gap-2">
              {items.map((item) => (
                <li
                  key={item.productId}
                  className="flex flex-col gap-2.5 rounded-2xl bg-paper-100 p-3"
                >
                  {/* Two rows, not one. Written as a single row first, and a
                      look at it on a 390px phone showed the product names cut to
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-1 rounded-full bg-paper-50 p-1 ring-1 ring-ink-950/10">
                      <button
                        onClick={() => setQty(item.productId, item.qty - 1)}
                        aria-label={`One less ${item.name}`}
                        className="grid h-9 w-9 place-items-center rounded-full text-lg font-black text-ink-900 transition-colors hover:bg-ink-950 hover:text-paper-50"
                      >
                        −
                      </button>
                      <span
                      ...
                      <button
                        ...
                        className="grid h-9 w-9 place-items-center rounded-full text-lg font-black text-ink-900 transition-colors hover:bg-brand-700 hover:text-paper-50"
                      >
                        +
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="ml-auto shrink-0 rounded-full px-2 py-1 text-xs font-bold text-ink-900/45 transition-colors hover:text-brand-700"
                    >
                      Remove
                    </button>
                  </div>
            ...
            <div className="flex flex-col-reverse gap-2 sm:flex-row">
              ...
              <button
                ...
                className="rounded-full px-5 py-3 font-bold text-ink-900/70 transition-colors hover:bg-paper-100 hover:text-ink-950 sm:flex-1"
              >
                Add more
              </button>
              <button
                ...
                className="rounded-full bg-brand-700 px-5 py-3 font-display text-lg font-black text-paper-50 transition-colors hover:bg-brand-800 sm:flex-[2]"
              >
                Confirm — {money(total)}
              </button>
            </div>
