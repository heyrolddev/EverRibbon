    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-ink-900/70">
            {open.length > 0
        ...
        {resolved.length > 0 && (
          <div className="flex shrink-0 flex-wrap gap-2">
            <button
              ...
              className="rounded-full bg-paper-50 px-3.5 py-1.5 text-xs font-bold text-ink-900/70 ring-1 ring-ink-950/10 transition-colors hover:bg-ink-950 hover:text-paper-50"
            >
              {showResolved
            ...
            {showResolved && (
              <button
                ...
                disabled={pending}
                className="rounded-full px-3.5 py-1.5 text-xs font-bold text-ink-900/60 transition-colors hover:text-brand-700 disabled:opacity-60"
              >
                Delete these
              </button>
            )}
      ...
      <ul className="mt-5 flex flex-col gap-2">
        {shown.map((e) => (
          <li
            key={e.id}
            className="rounded-2xl bg-paper-50 p-4 ring-1 ring-ink-950/10"
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-ink-950">{e.message}</p>
                <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-ink-900/55">
                  {e.route && (
                    <code className="font-mono font-bold text-ink-900/75">
                      {e.route}
                    </code>
                  )}
                  <span>
                  ...
                  <span className="font-bold text-ink-900/75">
              ...
              <button
                ...
                className={`shrink-0 rounded-full px-3.5 py-1.5 text-xs font-bold transition-colors disabled:opacity-60 ${
                  e.resolved
                    ? "bg-ink-950/5 text-ink-900/70 hover:bg-ink-950 hover:text-paper-50"
            ...
            {e.stack && (
              <details className="mt-3">
                <summary className="cursor-pointer text-xs font-semibold text-ink-900/50">
                  Details for whoever fixes it
                </summary>
                <pre className="mt-2 max-h-56 overflow-auto rounded-xl bg-ink-950/5 p-3 text-[11px] leading-relaxed text-ink-900/75">
                  {e.stack}
                </pre>
              </details>
            )}
      ...
      {shown.length === 0 && (
        <p className="mt-5 text-sm text-ink-900/55">
