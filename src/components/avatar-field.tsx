...
export function AvatarField({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            disabled={pending}
            className="rounded-full bg-ink-950 px-4 py-2 text-xs font-bold text-paper-50 transition-colors hover:bg-brand-700 disabled:opacity-50"
          >
          ...
          {avatarUrl && !pending && (
            <button
              type="button"
              onClick={clear}
              className="rounded-full px-4 py-2 text-xs font-bold text-brand-700 transition-colors hover:bg-brand-700 hover:text-paper-50"
            >
              Remove
            </button>
          )}
      ...
      {error ? (
        <p className="rounded-2xl bg-brand-50 px-4 py-2.5 text-xs font-semibold text-brand-800">
          {error}
        </p>
      ) : (
        <p className="text-[11px] font-medium text-ink-900/50">
