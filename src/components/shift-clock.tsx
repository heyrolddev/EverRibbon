    ...
    return (
      <>
        <button
          onClick={begin}
          disabled={busy}
          className="flex w-full items-center gap-3 rounded-xl bg-ok-600/20 px-3 py-2.5 text-sm font-bold text-ok-400 transition-colors hover:bg-ok-600 hover:text-paper-50 disabled:opacity-60"
        >
  ...
  return (
    <>
      <button
        ...
        className="flex w-full items-center gap-3 rounded-xl bg-paper-50/10 px-3 py-2.5 text-sm font-bold text-paper-100 transition-colors hover:bg-brand-700 hover:text-paper-50"
      >
        <span
          aria-hidden
          className="h-2 w-2 shrink-0 animate-pulse rounded-full bg-ok-500"
        />
        <span className="min-w-0 flex-1 text-left">On shift</span>
        <span className="shrink-0 text-xs font-black tabular-nums text-accent-200">
          {elapsed}
        </span>
      </button>
        ...
        >
          <div className="flex flex-col gap-4">
            {/* Required, and the hint says why rather than just that it is.
                ...
                to test it against. */}
            <Field
              label="Cash in the drawer ({brand.currency.symbol})"
