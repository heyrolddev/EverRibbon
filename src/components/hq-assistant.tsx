  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-4">
        ...
        {turns.map((turn, i) =>
          turn.role === "you" ? (
            <p
              key={i}
              className="max-w-[85%] self-end rounded-3xl rounded-br-lg bg-ink-950 px-5 py-3 text-paper-50"
            >
              {turn.text}
            </p>
          ) : (
        ...
        {pending && (
          <p className="max-w-[85%] rounded-3xl rounded-bl-lg bg-paper-100 px-5 py-3 text-sm text-ink-900/60 ring-1 ring-ink-950/10">
            Working it out…
          </p>
        )}
        <div ref={endRef} />
      ...
      <form
        ...
        className="sticky bottom-4 flex gap-2 rounded-3xl bg-paper-50 p-2 shadow-lg ring-1 ring-ink-950/10"
      >
        <input
          value={draft}
          onChange={(e) => setDraft(e.target.value)}
          placeholder="Ask anything — “paano naging ganyan ang net profit?”"
          className="min-w-0 flex-1 rounded-2xl bg-paper-100 px-4 py-3 text-ink-950 outline-none ring-1 ring-ink-950/10 focus:ring-2 focus:ring-accent-200"
        />
        <button
          type="submit"
          disabled={pending || !draft.trim()}
          className="shrink-0 rounded-2xl bg-brand-700 px-5 py-3 font-bold text-paper-50 transition-colors hover:bg-brand-800 disabled:bg-ink-950/15 disabled:text-ink-900/40"
        >
          Ask
        </button>
      </form>
      ...
      <div>
        <button
          ...
          className="text-xs font-black uppercase tracking-widest text-ink-900/50 hover:text-brand-700"
        >
                ...
                <ul className="mt-1.5 flex flex-col gap-1">
                  {topics.map((t) => (
                    <li key={t.id}>
                      <button
                        onClick={() => pick(t)}
                        className="text-left text-sm text-ink-900/75 hover:text-brand-700 hover:underline"
                      >
                        {t.question}
                      </button>
                    </li>
...
function Intro({ onPick }: { onPick: (t: GuideTopic) => void }) {
  return (
    <div className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10">
      ...
      <p className="mt-3 max-w-2xl text-xs text-ink-900/50">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {suggestions().map((t) => (
          <button
            key={t.id}
            onClick={() => onPick(t)}
            className="rounded-full bg-paper-50 px-4 py-2 text-sm font-semibold text-ink-900 ring-1 ring-ink-950/10 transition-colors hover:bg-accent-200 hover:text-ink-950"
          >
            {t.question}
          </button>
        ))}
...
export function Reply({ answer }: { answer: Answer }) {
  return (
    <div className="max-w-[92%] rounded-3xl rounded-bl-lg bg-paper-100 p-5 ring-1 ring-ink-950/10">
      <p className="whitespace-pre-line text-ink-900/85">{answer.text}</p>
      ...
      {answer.numbers && (
        ...
        <pre className="mt-4 whitespace-pre-wrap break-words rounded-2xl bg-ink-950 p-4 font-mono text-[12.5px] leading-relaxed text-paper-100">
          {answer.numbers}
        </pre>
      )}
      ...
      {answer.where && (
        <Link
          ...
          className="mt-4 inline-block rounded-full bg-ink-950 px-4 py-2 text-xs font-black uppercase tracking-widest text-accent-200 transition-transform hover:scale-105"
        >
