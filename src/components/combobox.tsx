  ...
  return (
    <div ref={boxRef} className="relative">
      <input
        ...
        role="combobox"
        className="w-full rounded-xl border-2 border-ink-950/10 bg-paper-100 px-4 py-2 text-ink-950 outline-none transition-colors placeholder:text-ink-900/35 focus:border-accent-200"
      />
      ...
      {open && (
        <ul
          id={listId}
          role="listbox"
          className="absolute z-50 mt-1 max-h-64 w-full overflow-y-auto rounded-xl bg-paper-50 py-1 shadow-2xl ring-1 ring-ink-950/15"
        >
          {matches.length === 0 && (
            <li className="px-4 py-3 text-sm text-ink-900/50">
            ...
            <li key={o.value}>
              ...
              <button
                ...
                className={`flex w-full items-baseline justify-between gap-3 px-4 py-2 text-left text-sm transition-colors ${
                  i === active ? "bg-brand-700 text-paper-50" : "text-ink-900/85"
                }`}
              >
