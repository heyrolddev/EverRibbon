...
export function PageHeader({
  ...
  return (
    <section className="under-nav grain relative overflow-hidden bg-ink-950">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -left-20 -top-24 h-64 w-64 rounded-full bg-brand-700/40 blur-3xl drift"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -right-10 bottom-0 h-56 w-56 rounded-full bg-accent-200/20 blur-3xl drift"
      />
      ...
      >
        <Reveal>
          {eyebrow && (
            <span className="inline-block rounded-full bg-accent-200 px-4 py-1 text-xs font-bold uppercase tracking-widest text-ink-950">
              {eyebrow}
            </span>
          )}
          <h1
            className={`font-display font-black tracking-tight text-paper-50 ${
              compact
          ...
          {subtitle && (
            <p
              className={`max-w-xl text-paper-100/70 ${
