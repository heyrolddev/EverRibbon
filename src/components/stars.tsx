...
export function Stars({
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 text-accent-300 ${className}`}
      role="img"
...
export function StarPicker({
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      <div className="flex items-center gap-1" onMouseLeave={() => setHover(0)}>
        {[1, 2, 3, 4, 5].map((n) => (
          <button
            ...
            onFocus={() => setHover(n)}
            onClick={() => onChange(n)}
            className={`text-accent-300 transition-transform disabled:opacity-50 ${
      ...
      {shown > 0 && (
        <span className="text-sm font-bold text-ink-900">{WORDS[shown]}</span>
      )}
    </div>
  );
}
