  ...
  return (
    <fieldset
      className={`flex flex-col gap-3 rounded-3xl p-5 ring-1 ${
        mustSchedule ? "bg-accent-50 ring-accent-200/60" : "bg-paper-100 ring-ink-950/10"
      }`}
    >
      <legend className="px-1 text-xs font-bold uppercase tracking-widest text-ink-900">
        When do you want it?
      </legend>

      {!canOrderNow && (
        <p className="rounded-2xl bg-paper-50 px-4 py-3 text-sm font-semibold text-ink-900 ring-1 ring-ink-950/10">
          {state.reason}
          {state.opensNext && (
            <span className="block font-normal text-ink-900/65">
              {state.opensNext}
            </span>
          )}
          {settings.accepting_orders && (
            <span className="mt-1 block font-normal text-ink-900/65">
      ...
      {value !== null && (
        <div className="flex flex-col gap-2">
          <input
            ...
            className="self-start rounded-2xl border-2 border-ink-950/15 bg-paper-50 px-4 py-2.5 text-sm font-semibold text-ink-950 outline-none focus:border-brand-700"
          />
          {check && !check.ok ? (
            <p className="text-sm font-semibold text-brand-800">{check.reason}</p>
          ) : (
