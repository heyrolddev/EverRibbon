...
const field =
  "rounded-xl border-2 border-ink-950/15 bg-paper-50 px-4 py-2.5 text-sm text-ink-950 outline-none transition-colors focus:border-brand-700";
const label = "flex flex-col gap-1.5 text-[11px] font-bold uppercase tracking-wide text-ink-900/70";
 ...
 */
export function RelayedReviewForm({ products }: { products: MealChoice[] }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [authorName, setAuthorName] = useState("");
  const [productId, setMealId] = useState("");
    ...
    return (
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          className="rounded-full bg-ink-950 px-5 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-700"
        >
          ＋ Add a review from Messenger
        </button>
        {saved && (
          <span className="text-sm font-bold text-ok-700">Added ✓</span>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <label className={label}>
          ...
          <input
            ...
            className={field}
          />
          <span className="text-[10px] font-medium normal-case tracking-normal text-ink-900/45">
