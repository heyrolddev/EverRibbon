...
function ageOf(iso: string | null) {
  ...
  const when = new Date(iso).toLocaleString(brand.locale, {
        dateStyle: "medium",
    timeStyle: "short",
  });
  return { days, when };
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="shrink-0">
            <button
              ...
              disabled={busy !== null}
              className="w-full rounded-2xl bg-ink-950 px-7 py-4 text-left font-display text-lg font-black text-paper-50 shadow-lg transition-transform hover:-translate-y-0.5 disabled:translate-y-0 disabled:opacity-60 lg:w-auto"
            >
              {busy === "full.json" ? (
                <>Packing it up…</>
              ) : (
              ...
              <span className="mt-0.5 block text-xs font-semibold opacity-60">
                {totalRows.toLocaleString(brand.locale)} rows · one .json file
              </span>
            </button>
          </div>
        </div>
      ...
      {brokenTables.length > 0 && (
        <p className="rounded-2xl bg-accent-200 px-5 py-4 text-sm text-ink-950">
        ...
        <ul className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {files.map((f) => {
            ...
            return (
              <li
                key={f.kind}
                className="flex flex-col rounded-3xl bg-paper-100 p-5 ring-1 ring-ink-950/10"
              >
                <div className="flex items-start justify-between gap-3">
                  <h4 className="font-display text-lg font-black text-ink-950">
                    {f.label}
                  </h4>
                  <span className="shrink-0 rounded-full bg-ink-950/5 px-2.5 py-1 text-[11px] font-bold tabular-nums text-ink-900/60">
                    {rows.toLocaleString(brand.locale)}
                  </span>
                </div>
                <p className="mt-2 text-sm text-ink-900/75">{f.what}</p>
                <p className="mt-1.5 flex-1 text-xs text-ink-900/50">{f.when}</p>

                {f.sensitive && (
                  <p className="mt-3 rounded-xl bg-brand-700/10 px-3 py-2 text-[11px] font-semibold text-brand-800">
                ...
                <button
                  ...
                  className="mt-4 rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-ink-900 disabled:cursor-not-allowed disabled:bg-ink-950/20 disabled:text-ink-900/40"
                >
                  {busy === f.kind
                    ? "Preparing…"
                    : empty
      ...
      <details className="rounded-3xl bg-paper-100 ring-1 ring-ink-950/10">
        <summary className="cursor-pointer list-none rounded-3xl px-5 py-4 text-sm font-bold text-ink-950">
          ...
          <span className="ml-2 font-normal text-ink-900/50">
            {counts.length} tables · {totalRows.toLocaleString(brand.locale)} rows
          </span>
        </summary>
        <ul className="grid gap-x-6 gap-y-1 border-t border-ink-950/10 px-5 py-4 sm:grid-cols-2 lg:grid-cols-3">
          {counts.map((c) => (
            ...
            >
              <span className="truncate font-mono text-xs text-ink-900/70">
                {c.table}
              </span>
              <span
              ...
              >
                {c.error ? "failed" : c.count.toLocaleString(brand.locale)}
              </span>
            </li>
          ))}
        </ul>
      ...
      <section
        className={`rounded-3xl p-6 ring-1 ${
          health.length === 0
            ? "bg-ok-600/10 ring-ok-600/25"
            : "bg-accent-200/25 ring-accent-500/30"
        }`}
      >
        <h3 className="font-display text-lg font-black text-ink-950">
          {health.length === 0
        ...
        <p className="mt-1 max-w-2xl text-sm text-ink-900/60">
          {health.length === 0
            ? "Every recipe points at something that exists, and no two products share a name."
            : "None of this stops the shop working — but each one makes a number somewhere quietly wrong."}
        </p>
        {health.length > 0 && (
          <ul className="mt-4 flex flex-col gap-1.5">
            {health.map((h, i) => (
              <li key={i} className="text-sm text-ink-900/80">
                <span className="mr-2 rounded-full bg-ink-950/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-ink-900/70">
                  {h.kind}
                </span>
                {h.detail}
              </li>
      ...
      <section className="rounded-3xl border-2 border-dashed border-ink-950/15 p-6">
        ...
        <ol className="mt-3 flex flex-col gap-2 text-sm text-ink-900/75">
          <li>
