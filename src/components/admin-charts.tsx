...
const HUES = {
  money: { fill: "bg-brand-700", soft: "bg-brand-700/15" },
  count: { fill: "bg-ok-600", soft: "bg-ok-600/15" },
} as const;
...
export function ColumnChart({
  ...
  const fmt = (n: number) => (format === "money" ? money(n) : String(n));
    ...
    >
      {data.map((d, i) => (
        ...
        >
          ...
          <span className="pointer-events-none absolute bottom-full z-10 mb-1 hidden whitespace-nowrap rounded-lg bg-ink-950 px-2 py-1 text-[11px] font-bold text-paper-50 shadow-lg group-hover:block">
          ...
          <span className="truncate text-[10px] font-semibold text-ink-900/50">
            {d.label}
          </span>
        </div>
      ))}
      ...
      {data.map((d) => (
        <li key={d.label} className="group flex items-center gap-3">
          ...
          <span className="flex h-5 flex-1 items-center">
            ...
            {suffix && (
              <span className="ml-2 whitespace-nowrap text-xs text-ink-900/55">
                {suffix(d)}
              </span>
            )}
          </span>
