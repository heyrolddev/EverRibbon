...
const fieldClass =
  "w-full rounded-2xl border-2 border-ink-950/15 bg-paper-100 px-5 py-3 font-normal text-ink-950 outline-none transition-colors placeholder:text-ink-900/40 focus:border-brand-700";
...
export function AddressField({
  ...
  return (
    <div className="relative flex flex-col gap-2">
      <label className="flex flex-col gap-2 text-xs font-bold uppercase tracking-widest text-ink-900">
        {label} {required && <span className="text-brand-700">*required</span>}
        <textarea
          required={required}
          value={value}
          onChange={(e) => {
      ...
      <p className="-mt-1 text-[11px] font-medium text-ink-900/55">
        {searching
      ...
      {open && hits.length > 0 && (
        <div className="rounded-2xl bg-paper-50 p-2 ring-1 ring-ink-950/10">
          <p className="px-2 py-1 text-[11px] font-bold uppercase tracking-widest text-ink-900/50">
            Did you mean
          </p>
            ...
            {hits.map((h) => (
              <li key={`${h.lat},${h.lng}`}>
                <button
                  ...
                  className="w-full rounded-xl px-3 py-2 text-left text-sm text-ink-900 transition-colors hover:bg-brand-700 hover:text-paper-50"
                >
                  📍 {h.label}
                </button>
              </li>
          ...
          <button
            ...
            className="mt-1 w-full rounded-xl px-3 py-1.5 text-xs font-bold text-ink-900/60 hover:text-brand-700"
          >
