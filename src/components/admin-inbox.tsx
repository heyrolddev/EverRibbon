 ...
 */
/*
 * Messenger's own blue, which belongs to Messenger.
 *
 * A channel badge has to look like the channel, so this is not a colour
 * the shop gets to pick and not a theme token. Named rather than inlined so
 * it reads as a deliberate exception.
 */
const MESSENGER_BLUE = "#0084ff"; // brand-literal-ok: another company's mark
      ...
      <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-bold transition-colors ${
                  filter === f.key
                    ...
                    : "bg-paper-100 text-ink-900 ring-1 ring-ink-950/10 hover:bg-paper-200"
                }`}
              >
                {f.label}
        ...
        <span
          className={`ml-auto flex items-center gap-1.5 text-xs font-bold ${
            connected ? "text-ok-700" : "text-ink-900/40"
          }`}
        >
          <span
            className={`h-2 w-2 rounded-full ${
              connected ? "bg-ok-600" : "bg-ink-900/30"
            }`}
          />
          ...
          shown.length === 0 ? (
            ...
            >
              {filter === "waiting"
                ...
                : "When someone asks ${brand.name} something, it lands here."}
            </EmptyState>
          ) : (
            <ul className="flex flex-col gap-3">
              {shown.map((t) => {
                ...
                return (
                  <li
                    key={t.id}
                    className={`overflow-hidden rounded-3xl bg-paper-50 ring-1 transition-shadow ${
                      t.needsHuman && !t.handled
                        ? "ring-2 ring-brand-700/40"
                        : "ring-ink-950/10"
                    }`}
                  >
                    <button
                    ...
                    >
                      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-ink-950 text-sm font-black text-accent-200">
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex flex-wrap items-center gap-2">
                          ...
                          {t.channel === "messenger" && (
                            <span className={`rounded-full bg-[${MESSENGER_BLUE}]/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[${MESSENGER_BLUE}]`}>
                              Messenger
                            </span>
                          )}
                          {t.needsHuman && !t.handled && (
                            <span className="rounded-full bg-brand-700 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-paper-50">
                              Needs a reply
                            </span>
                          )}
                          {t.handled && (
                            <span className="rounded-full bg-ok-600/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-ok-700">
                              Handled
                            </span>
                          )}
                        </span>
                        <span className="mt-0.5 block truncate text-sm text-ink-900/70">
                      ...
                      <span className="shrink-0 text-ink-900/40">
                        {open ? "▲" : "▼"}
                      </span>
                    </button>

                    {open && (
                      <div className="border-t border-ink-950/10 bg-paper-100 px-5 py-4">
                          ...
                          {t.messages.map((m, i) => (
                            <li
                              ...
                              className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${
                                m.role === "user"
                                  ? "self-start bg-paper-50 text-ink-950 ring-1 ring-ink-950/10"
                                  : "self-end bg-ink-950 text-paper-100"
                              }`}
                            >
                              {m.content}
                        ...
                        >
                          <div className="flex items-end gap-2">
                            <textarea
                              ...
                              className="flex-1 resize-none rounded-2xl border-2 border-ink-950/15 bg-paper-50 px-4 py-2.5 text-sm outline-none focus:border-brand-700"
                            />
                            <button
                              type="submit"
                              disabled={sending || !draft.trim()}
                              className="shrink-0 rounded-full bg-brand-700 px-5 py-2.5 text-sm font-bold text-paper-50 disabled:opacity-50"
                            >
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          <button
                            ...
                            className={`rounded-full px-4 py-2 text-xs font-bold transition-colors disabled:opacity-60 ${
                              t.handled
                                ? "bg-paper-50 text-ink-900 ring-1 ring-ink-950/15"
...
/**
 * "the shop assistant got this wrong — here's the right answer."
 *
...
function TeachPanel({
  ...
  return (
    <div className="mt-3 flex flex-col gap-3 rounded-2xl bg-accent-50 p-4 ring-1 ring-accent-200/50">
      <p className="text-sm font-bold text-ink-950">
        Teach Ask {brand.name} this answer
      </p>
      ...
      <label className="flex flex-col gap-1 text-xs font-bold uppercase tracking-widest text-ink-900">
        The question
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          className="rounded-xl border-2 border-ink-950/15 bg-paper-50 px-3 py-2 text-sm font-normal normal-case tracking-normal outline-none focus:border-brand-700"
        />
      </label>
      ...
      <label className="flex flex-col gap-1 text-xs font-bold uppercase tracking-widest text-ink-900">
        ...
        <textarea
          ...
          className="resize-none rounded-xl border-2 border-ink-950/15 bg-paper-50 px-3 py-2 text-sm font-normal normal-case tracking-normal outline-none focus:border-brand-700"
        />
      </label>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <span className="text-[11px] font-semibold text-ink-900/55">
          Will trigger on:
        </span>
        {preview.length === 0 ? (
          ...
          preview.map((t) => (
            <span
              key={t}
              className="rounded-full bg-ink-950 px-2 py-0.5 font-mono text-[10px] font-bold text-accent-200"
            >
              {t}
            </span>
          ))
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className="rounded-full bg-brand-700 px-5 py-2 text-xs font-bold text-paper-50 disabled:opacity-50"
        >
