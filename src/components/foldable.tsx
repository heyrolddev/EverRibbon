...
export function Foldable({
  /** Tailwind background+text for this row's status, e.g. "bg-brand-700 text-paper-50". */
  chip,
  /** Tailwind border colour for the left rail, e.g. "border-accent-200". */
  rail,
    ...
    return (
      <li>
        <button
          ...
          aria-expanded={false}
          className={`flex w-full items-center gap-3 overflow-hidden rounded-2xl border-l-[6px] bg-paper-100 py-2.5 pl-3 pr-3 text-left ring-1 ring-ink-950/10 transition-colors hover:bg-paper-200/70 ${rail}`}
        >
          {folded}
        </button>
      </li>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="shrink-0 rounded-full bg-ink-950 px-3 py-1 text-[11px] font-black uppercase tracking-wide text-paper-50">
          Close ✕
        </span>
      </button>
      {children}
