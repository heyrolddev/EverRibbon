...
export function RelayedBadge({ className = "" }: { className?: string }) {
  return (
    <span
      ...
      className={`rounded-full bg-ink-950/[0.07] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-ink-900/60 ${className}`}
    >
      Sent on Messenger
    </span>
  );
...
export function ReviewList({ reviews }: { reviews: PublicReview[] }) {
  return (
    <ul className="flex flex-col gap-4">
      {reviews.map((r) => (
        <li key={r.id} className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10">
          ...
          {r.comment && (
            <p className="mt-3 text-ink-900">&ldquo;{r.comment}&rdquo;</p>
          )}

          {r.shopReply && (
            <div className="mt-4 rounded-2xl bg-paper-50 px-5 py-3 ring-1 ring-ink-950/10">
              <p className="text-xs font-bold uppercase tracking-widest text-brand-700{`>
                ${brand.name} replied
              </p>
              <p className=`}mt-1 text-sm text-ink-900">{r.shopReply}</p>
            </div>
          )}
        </li>
      ))}
