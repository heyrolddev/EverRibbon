...
function ShiftCard({ r }: { r: ShiftReport }) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl bg-paper-100 ring-1 ring-ink-950/10">
      <button
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        ...
        <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
          <div className="min-w-0">
            <p className="font-display text-lg font-black leading-tight text-ink-950">
              {r.staffName}
              {running && (
                <span className="ml-2 inline-flex items-center gap-1.5 rounded-full bg-ok-600 px-2.5 py-0.5 align-middle text-[10px] font-black uppercase tracking-wide text-paper-50">
                  <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-paper-50" />
                  On shift
                </span>
              )}
            </p>
            <p className="mt-0.5 text-xs text-ink-900/55">
              ...
              <span className="ml-2 font-bold text-ink-900/70">{r.length}</span>
            </p>
          </div>
          ...
          <div className="text-right">
            ...
            {diff !== null && (
              <p className="mt-0.5 text-xs font-bold">
                {square ? (
                  <span className="text-ok-700">Drawer square</span>
                ) : diff < 0 ? (
                  <span className="text-brand-700">
                    {money(Math.abs(diff))} short
                  </span>
                ) : (
                  <span className="text-warn-700">{money(diff)} over</span>
                )}
              </p>
            )}
            {diff === null && !running && (
              ...
              <p
                className={`mt-0.5 text-xs ${
                  r.autoClosed ? "font-bold text-brand-700" : "text-ink-900/40"
                }`}
              >
        ...
        <div className="border-t border-ink-950/10 bg-paper-50/70 px-5 py-4">
          <dl className="grid gap-x-6 gap-y-2 sm:grid-cols-2">
            <div className="flex justify-between border-b border-ink-950/5 pb-1.5">
              <dt className="text-sm text-ink-900/60">Taken in total</dt>
              <dd><Money n={r.takings} /></dd>
            </div>
            <div className="flex justify-between border-b border-ink-950/5 pb-1.5">
              <dt className="text-sm text-ink-900/60">Of that, cash</dt>
              <dd><Money n={r.cashExpected} /></dd>
            </div>
            <div className="flex justify-between border-b border-ink-950/5 pb-1.5">
              <dt className="text-sm text-ink-900/60">Counted in the drawer</dt>
              <dd>
                {r.closingCash === null ? (
            ...
            <div className="flex justify-between border-b border-ink-950/5 pb-1.5">
              <dt className="text-sm text-ink-900/60">Difference</dt>
              ...
              <dd>
                {diff === null ? (
                  <span className="text-sm text-ink-900/40">nothing to compare</span>
                ) : square ? (
                  <span className="font-display font-black text-ok-700">Square</span>
                ) : (
                  <span
                    className={`font-display font-black tabular-nums ${
                      diff < 0 ? "text-brand-700" : "text-warn-700"
                    }`}
                  >
                    {money(Math.abs(diff))} {diff < 0 ? "short" : "over"}
                  </span>
                )}
              </dd>
            </div>
          ...
          {r.note && (
            <p className="mt-3 rounded-xl bg-accent-200/20 px-4 py-2.5 text-sm text-ink-950">
              &ldquo;{r.note}&rdquo;
            </p>
          )}
            ...
            <ul className="mt-4 flex flex-col gap-1">
              {r.actions.map((a, i) => (
                <li key={i} className="flex gap-3 text-sm">
                  <span className="w-28 shrink-0 tabular-nums text-ink-900/40">
                    {formatDateTime(a.at)}
                  </span>
                  <span className="text-ink-900/80">{a.description}</span>
                </li>
              ))}
            </ul>
          )}
    ...
    <>
      <button
        ...
        className={`shrink-0 rounded-xl px-4 py-2 text-xs font-black uppercase tracking-wide transition-colors disabled:opacity-60 ${
          tone === "red"
            ? "bg-ink-950/5 text-ink-900/70 hover:bg-brand-700 hover:text-paper-50"
            : tone === "gold"
              ? "bg-accent-200 text-ink-950 ring-2 ring-ink-950 hover:bg-accent-300"
      ...
      {error && (
        <p className="w-full text-xs font-semibold text-brand-700">{error}</p>
      )}
    </>
  );
}
  ...
  return (
    <>
      <button
        ...
        className="shrink-0 rounded-xl bg-ink-950/5 px-4 py-2 text-xs font-black uppercase tracking-wide text-ink-900/70 transition-colors hover:bg-ink-950 hover:text-paper-50"
      >
        Email
      </button>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="rounded-full bg-ink-950 px-5 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-700 disabled:opacity-60"
              >
              ...
              <button
                ...
                className="rounded-full px-5 py-2.5 text-sm font-bold text-ink-900/70 transition-colors hover:text-ink-950 disabled:opacity-60"
              >
                Cancel
              </button>
            </div>
  ...
  return (
    <>
      <button
        ...
        className="shrink-0 rounded-xl px-4 py-2 text-xs font-black uppercase tracking-wide text-brand-700 ring-1 ring-brand-700/30 transition-colors hover:bg-brand-700 hover:text-paper-50"
      >
        Delete
      </button>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="rounded-full bg-brand-700 px-5 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-800 disabled:opacity-60"
              >
      ...
      <section>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="rounded-2xl bg-ink-950 px-5 py-2.5 text-sm font-black text-paper-50 transition-colors hover:bg-ink-900"
          >
        ...
        <ul className="mt-4 flex flex-col gap-2">
          {people.map((p) => (
            <li
              key={p.id}
              className="flex flex-wrap items-center gap-3 rounded-2xl bg-paper-100 px-4 py-3 ring-1 ring-ink-950/10"
            >
              <div className="min-w-0 flex-1">
                <p className="font-bold text-ink-950">
                  {p.name ?? "No name set"}
                  {p.onShift && (
                    <span className="ml-2 rounded-full bg-ok-600 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-paper-50">
                      On shift
                    </span>
                  )}
                </p>
                ...
                {p.role !== "customer" && (
                  <p className="mt-0.5 max-w-md text-xs text-ink-900/40">
                    {ROLE_BLURBS[p.role]}
                  </p>
                )}
              </div>
            ...
            {candidates.length === 0 ? (
              ...
              <ul className="mt-4 flex flex-col gap-2">
                {candidates.slice(0, 25).map((p) => (
                  <li
                    key={p.id}
                    className="flex flex-wrap items-center gap-3 rounded-2xl bg-paper-100 px-4 py-2.5 ring-1 ring-ink-950/10"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-bold text-ink-950">
                        {p.name ?? "No name set"}
                      </p>
                      <p className="text-xs text-ink-900/50">{p.phone ?? "No number"}</p>
                    </div>
                    <div className="flex shrink-0 flex-wrap gap-1.5">
