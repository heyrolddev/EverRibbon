...
export function CustomerRow({
  ...
  return (
    <li className="rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          {customer.address && (
            <p className="mt-0.5 text-sm text-ink-900/55">{customer.address}</p>
          )}
          <p className="mt-1 text-xs font-semibold text-ink-900/70">
            {customer.orderCount} order{customer.orderCount === 1 ? "" : "s"} ·{" "}
            {customer.completedCount} completed · {money(customer.totalSpent)} spent
          </p>
        </div>
        ...
        {canManage && (
          <div className="flex flex-col items-end gap-2">
            ...
            {error && (
              <span className="text-xs font-semibold text-brand-800">{error}</span>
            )}
          </div>
        )}
      </div>
