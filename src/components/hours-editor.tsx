...
const field =
  "rounded-xl border-2 border-ink-950/15 bg-paper-50 px-3 py-2 text-sm font-semibold text-ink-950 outline-none focus:border-brand-700";
...
export function HoursEditor({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-3 rounded-3xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
        <div>
            ...
            return (
              <li
                ...
                className={`flex flex-wrap items-center gap-3 rounded-2xl px-4 py-3 ${
                  day.is_open ? "bg-paper-50" : "bg-paper-50/50"
                ...
                {day.is_open ? (
                  <>
                    <input
                      ...
                      className={field}
                    />
                    <span className="text-ink-900/50">to</span>
                    <input
                      type="time"
                  ...
                  <span className="text-sm font-semibold text-ink-900/50">Closed</span>
                )}
              </li>
            );
          })}
        ...
        <button
          ...
          className={`self-start rounded-full px-6 py-3 font-bold transition-colors disabled:opacity-60 ${
            savedDays ? "bg-ok-600 text-paper-50" : "bg-brand-700 text-paper-50"
          }`}
        >
  ...
  return (
    <section
      className={`flex flex-col gap-4 rounded-3xl p-5 ring-2 ${
        accepting ? "bg-paper-100 ring-ok-600/30" : "bg-brand-50 ring-brand-700"
      }`}
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 max-w-md text-sm text-ink-900/65">
            {accepting
        ...
        <button
          ...
          className={`shrink-0 rounded-full px-6 py-3 font-bold transition-transform hover:scale-105 disabled:opacity-60 ${
            accepting ? "bg-brand-700 text-paper-50" : "bg-ok-600 text-paper-50"
          }`}
        >
      ...
      <div className="flex flex-wrap gap-5">
        ...
        <button
          ...
          className={`rounded-full px-5 py-2 text-sm font-bold transition-colors disabled:opacity-60 ${
            saved ? "bg-ok-600 text-paper-50" : "bg-ink-950 text-paper-50"
          }`}
        >
    ...
    <section className="flex flex-col gap-3 rounded-3xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <button
          onClick={add}
          disabled={busy || !date}
          className="rounded-full bg-ink-950 px-5 py-2.5 text-sm font-bold text-paper-50 disabled:opacity-50"
        >
          Add
        </button>
      </div>
      ...
      {closures.length === 0 ? (
        <p className="text-sm text-ink-900/50">Nothing coming up.</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {closures.map((c) => (
            <li
              key={c.closed_on}
              className="flex flex-wrap items-center gap-3 rounded-2xl bg-paper-50 px-4 py-2.5 ring-1 ring-ink-950/10"
            >
              <span className="font-bold text-ink-950">{formatDate(c.closed_on)}</span>
              {c.reason && (
                <span className="text-sm text-ink-900/65">{c.reason}</span>
              )}
              <button
                onClick={() => drop(c.closed_on)}
                disabled={busy}
                className="ml-auto text-xs font-bold text-brand-800 hover:underline disabled:opacity-50"
              >
                Remove
              </button>
            </li>
