  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <span className="flex items-center gap-2">
        {prefix && <span className="font-bold text-ink-900/60">{prefix}</span>}
        <input
          inputMode="decimal"
          value={value}
        ...
        />
        {suffix && (
          <span className="whitespace-nowrap text-sm font-semibold text-ink-900/60">
            {suffix}
          </span>
        )}
      </span>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
      ...
      <section className="rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
        ...
        <p className="mb-3 mt-1 text-sm text-ink-900/60">
      ...
      <section className="rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
        ...
        <p className="mb-4 mt-1 text-sm text-ink-900/60">
      ...
      <section className="rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
        ...
        <p className="mb-4 mt-1 text-sm text-ink-900/60">
          Based on the numbers above, on a {moneyRound(300)} order.
        </p>
        <ul className="flex flex-wrap gap-3">
          {preview.map(({ km, quote }) => (
            ...
            >
              <span className="block text-xs font-bold uppercase tracking-wide text-ink-900/55">
                ~{km} km
              </span>
              <span
                className={`font-display text-xl font-black ${
                  quote.ok ? "text-brand-700" : "text-ink-900/50"
                }`}
              >
                {quote.ok ? (quote.fee === 0 ? "Free" : money(quote.fee)) : "Too far"}
              </span>
            </li>
          ))}
        </ul>
