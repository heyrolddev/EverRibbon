...
export type DishRow = {
  ...
  sold: number;
  /** Null until something has sold, or when the product can't be costed. */
  menuClass: MenuClass | null;
  /** What this product costs to send out in a box, on top of the food. */
  packagingCost: number;
  packaging: { refType: "inv" | "production_run"; refId: string; qty: number }[];
  /** The recipe as stored, so it can be edited rather than only read. */
  recipe: { refType: "inv" | "production_run"; refId: string; qty: number }[];
  price: number;
  categories: string[];
  onMenu: boolean;
  available: boolean;
...
/**
 * What every product earns, and where the money went.
 *
 ...
 *
 * Sorted worst-first by default. A list of 72 products sorted by name is a
...
> = {
  ...
  tight: {
    ...
    label: "Tight",
    blurb: "Materials eat over 40% of the price.",
  },
  ok: {
    bar: "bg-accent-200",
    chip: "bg-brand-700 text-paper-50",
  ...
  great: {
    bar: "bg-ok-500",
    chip: "bg-ok-500 text-paper-50",
    label: "Strong",
    blurb: "Materials are under a quarter of the price.",
  },
  unknown: {
    bar: "bg-ink-950/20",
    chip: "bg-ink-950/10 text-ink-900/70",
    label: "No recipe",
    blurb: "Nothing has been entered for this product yet.",
  },
};
...
/** One product's price, split into what it cost and what's left. */
function CostBar({ product }: { product: DishRow }) {
  const tone = TONES[product.verdict];
  if (!product.costed || product.foodCostPct === null) {
    return (
      <div className="h-2.5 w-full rounded-full bg-ink-950/10" aria-hidden />
    );
  }
  // Capped so a product costing three times its price still draws a bar rather
  // than a segment running off the card — the red and the "Losing money" chip
  // already say it's past the end.
  const filled = Math.min(product.foodCostPct, 100);
  return (
    <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-ok-500/25">
      <div
...
function Stat({
  ...
  const skin = {
    ...
    warn: "bg-brand-700 text-paper-50 ring-accent-500/30",
...
export function DishCosts({
  ...
  classified: boolean;
  /** Charged once per take-out order — the bag — not per product. */
  orderPackagingCost: number;
  /** Category colours, so this screen reads the same way the menu does. */
  known?: MenuCategory[];
  orderPackaging: { refType: "inv" | "production_run"; refId: string; qty: number }[];
  failed: string[];
}) {
  ...
  // Which editor is open on a product: its recipe, or what it travels in.
  ...
  const summary = useMemo(() => {
    const costed = products.filter((d) => d.costed && d.price > 0);
    const uncosted = products.filter((d) => !d.costed);
    const attention = products.filter(
      (d) => d.verdict === "losing" || d.verdict === "tight"
    );
    // Averaged across products rather than weighted by sales: this is a menu
  ...
  const shown = useMemo(() => {
    ...
    // Products with no recipe have no percentage to sort by, so they sit at the
  ...
  const FILTERS: { key: Filter; label: string; n: number }[] = [
    { key: "all", label: "Every product", n: products.length },
    { key: "attention", label: "Needs a look", n: summary.attention.length },
    { key: "menu", label: "On the menu", n: products.filter((d) => d.onMenu).length },
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h2 className={hqTitle}>Product costs</h2>
        <p className="mt-1 max-w-2xl text-sm text-ink-900/60">
          What each product costs you to make, worked out from the recipes and
          material prices you&apos;ve already entered — and what&apos;s left
          over once it&apos;s sold.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        ...
        <Stat
          label="Best earner"
          value={summary.best ? money(summary.best.gross, 0) : "—"}
          sub={summary.best ? summary.best.name : "Nothing costed yet"}
        />
        <Stat
          label="Costed"
          value={`${summary.costed.length}/${products.length}`}
          sub={
      ...
      {/* One product, two ways of serving it — and the bag, which belongs to the
          order rather than to any product in it. */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl bg-paper-100 px-5 py-4 ring-1 ring-ink-950/10">
        <div>
          ...
          <p className="mt-0.5 text-sm text-ink-900/55">
            {orderPackaging.length === 0
              ? "Nothing yet — add the bag here rather than on each product, or a four-product order is charged four bags."
              : `${money(orderPackagingCost)} on top of the food, once per order.`}
          </p>
        </div>
        <button
          onClick={() => setEditingOrderPackaging(true)}
          className="shrink-0 rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-ink-900"
        >
        ...
        {classified && (
          <div className="flex flex-wrap gap-2">
            {(["star", "plowhorse", "puzzle", "dog"] as const).map((k) => {
              const n = products.filter((d) => d.menuClass === k).length;
              if (n === 0) return null;
      ...
      {classified &&
        (filter === "star" || filter === "plowhorse" || filter === "puzzle" || filter === "dog") && (
          <p className="rounded-2xl bg-paper-100 px-5 py-4 text-sm text-ink-900/80 ring-1 ring-ink-950/10">
        ...
        <ul className="flex flex-col gap-3">
          {shown.map((d) => {
            ...
            return (
              <li
                key={d.id}
                className="overflow-hidden rounded-3xl bg-paper-100 ring-1 ring-ink-950/10"
              >
                <button
                  onClick={() => setOpen(isOpen ? null : d.id)}
                  aria-expanded={isOpen}
                  className="w-full px-5 py-4 text-left transition-colors hover:bg-paper-200/60"
                >
                  <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-ink-900/50">
                  ...
                  <div className="mt-3 flex flex-wrap items-baseline gap-x-6 gap-y-1">
                    ...
                    <span className="text-sm text-ink-900/60">
                      ...
                      <strong className="font-display text-base tabular-nums text-ink-950">
                        {d.costed ? money(d.cost) : "—"}
                      </strong>
                    </span>
                    {/* The same product, two ways of serving it. Eaten here it
                        uses no packaging at all; in a box it costs more, and
                        that difference used to be a whole second product on the
                        menu. */}
                    {d.packagingCost > 0 && (
                      <span className="text-sm text-ink-900/60">
                        + box{" "}
                        <strong className="font-display text-base tabular-nums text-warn-700">
                          {money(d.packagingCost)}
                        </strong>
                      </span>
                    )}
                    ...
                    <span className="text-sm text-ink-900/60">
                      ...
                      <strong
                        className={`font-display text-base tabular-nums ${
                          d.gross < 0 ? "text-brand-700" : "text-ok-700"
                        }`}
                      >
                        {d.costed ? money(d.gross) : "—"}
                      </strong>
                  ...
                  <div className="mt-3">
                    ...
                    <p className="mt-1.5 text-xs text-ink-900/45">
                      {tone.blurb}
                      {d.problems.length > 0 && (
                        <span className="ml-1 font-bold text-warn-700">
                ...
                {isOpen && (
                  <div className="border-t border-ink-950/10 bg-paper-50/70 px-5 py-4">
                    {d.problems.length > 0 && (
                      <ul className="mb-4 flex flex-col gap-1 rounded-2xl bg-warn-500/10 px-4 py-3">
                        {d.problems.map((p) => (
                          <li key={p} className="text-xs font-semibold text-warn-700">
                            ⚠ {p}
                          </li>
                        ))}
                      </ul>
                    ...
                    {d.lines.length === 0 ? (
                      <p className="text-sm text-ink-900/60">
                        No recipe entered. Until there is one, this product
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full min-w-[420px] text-sm">
                          <thead>
                            <tr className="text-left text-[11px] font-black uppercase tracking-widest text-ink-900/40">
                            ...
                            {d.lines.map((l, i) => (
                              <tr key={`${l.label}-${i}`}>
                                <td className="py-2 pr-3">
                                  ...
                                  {l.kind !== "material" && (
                                    <span className="ml-2 rounded-full bg-ink-950/5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-ink-900/50">
                                      {l.kind === "production_run" ? "production_run" : "product"}
                                    </span>
                                  )}
                                  {l.problem && (
                                ...
                                <td className="py-2 text-right tabular-nums text-ink-900/70">
                                  {l.qty.toLocaleString(brand.locale)} {l.unit}
                                </td>
                                <td className="py-2 text-right tabular-nums text-ink-900/50">
                                  {l.unitCost > 0 ? money(l.unitCost, 4) : "—"}
                                </td>
                          ...
                          <tfoot>
                            <tr className="border-t-2 border-ink-950/15">
                              <td colSpan={3} className="pt-2 text-right font-bold text-ink-900/70">
                                Total materials
                              </td>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button
                        ...
                        className="rounded-xl bg-ink-950 px-4 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-ink-900"
                      >
                      ...
                      <button
                        ...
                        className="rounded-xl bg-warn-500/15 px-4 py-2.5 text-sm font-bold text-warn-700 transition-colors hover:bg-warn-500 hover:text-paper-50"
                      >
      ...
      {editing && editingWhat === "recipe" && (
        <RecipeEditor
          ...
          target={{ kind: "product", productId: editing.id }}
      ...
      {editing && editingWhat === "packaging" && (
        <RecipeEditor
          ...
          target={{ kind: "packaging", productId: editing.id }}
          onClose={() => setEditing(null)}
        />
      )}
      {editingOrderPackaging && (
        <RecipeEditor
          ...
          subtitle="Charged once per order, not once per product — the bag belongs here, not on each product."
          price={null}
          options={options}
