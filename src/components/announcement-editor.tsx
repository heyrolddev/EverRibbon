...
export function AnnouncementEditor({ rows }: { rows: Announcement[] }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div>
        <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-ink-900/40">
          On the homepage right now
        </p>
        <div className="overflow-hidden rounded-2xl border-y-4 border-ink-950 bg-brand-700 py-3">
          <p className="line-clamp-2 px-5 font-display text-lg font-black uppercase tracking-tight text-paper-50">
    ...
    <section>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 max-w-xl text-sm text-ink-900/60">{KIND_BLURB[kind]}</p>
          <p className="mt-1 max-w-xl text-xs text-ink-900/45">
            {HOME_LIMIT[kind] === 1
        ...
        <button
          onClick={onAdd}
          className="shrink-0 rounded-xl bg-ink-950 px-4 py-2 text-sm font-bold text-paper-50 transition-colors hover:bg-ink-900"
        >
          {KIND_ADD[kind]}
        </button>
      </div>
      ...
      {rows.length === 0 ? (
        <p className="mt-3 rounded-2xl border-2 border-dashed border-brand-300 bg-paper-100 p-5 text-sm text-ink-900/70">
          Nothing here yet.
        </p>
      ) : (
...
function Row({
  ...
  return (
    <li
      className={`rounded-2xl bg-paper-100 p-4 ring-1 ring-ink-950/10 ${
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
        ...
        <div className="min-w-0 flex-1">
          <p className="flex flex-wrap items-center gap-2">
            ...
            {row.pinned && (
              <span className="rounded-full bg-accent-200 px-2.5 py-0.5 text-[11px] font-black uppercase tracking-wide text-ink-950">
                ★ Pinned
              </span>
            )}
          </p>
          {row.body && (
            <p className="mt-1 max-w-2xl text-sm text-ink-900/65">{row.body}</p>
          )}
          <Window row={row} />
          {error && (
            <p className="mt-1 text-xs font-semibold text-brand-800">{error}</p>
          )}
        </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-1.5 sm:justify-end">
          <button
            ...
            className={`grid h-9 w-9 place-items-center rounded-lg text-base transition-colors disabled:opacity-40 ${
              row.pinned
                ? "bg-brand-700 text-paper-50 hover:bg-brand-800"
                : "bg-ink-950/5 text-ink-900/40 hover:bg-ink-950/10 hover:text-ink-900/70"
            }`}
          >
            {row.pinned ? "★" : "☆"}
          </button>
          ...
          {(row.kind === "promo" || isSlotKind(row.kind)) && (
            <>
              <button
                ...
                className="grid h-9 w-9 place-items-center rounded-lg bg-ink-950/5 font-black text-ink-900/60 transition-colors hover:bg-ink-950/10 disabled:opacity-30"
              >
                ↑
              </button>
              <button
          ...
          <button
            ...
            className={`rounded-lg px-3 py-2 text-xs font-black uppercase tracking-wide transition-colors disabled:opacity-50 ${
              row.is_active
                ? "bg-ink-950/5 text-ink-900/60 hover:bg-brand-700 hover:text-paper-50"
          ...
          <button
            onClick={onEdit}
            className="rounded-lg bg-ink-950/5 px-3 py-2 text-xs font-bold text-ink-900/70 transition-colors hover:bg-ink-950/10"
          >
            Edit
          </button>
        </div>
...
const PLACEHOLDER_BODY: Record<AnnouncementKind, string> = {
  promo: "e.g. One free hot coffee with any rice product, eaten at the stall.",
  news: "e.g. We're closed on the 5th for a private event. Back on the 6th.",
  dine_in: "e.g. Any hot coffee, with any rice product, eaten at the stall.",
  coming_soon: "e.g. Both landing before the end of the month.",
};

const field =
  "w-full rounded-xl border-2 border-ink-950/10 bg-paper-100 px-4 py-2.5 text-ink-950 outline-none transition-colors focus:border-accent-200";
...
function Editor({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink-950/50 p-0 sm:items-center sm:p-6">
      <div className="max-h-full w-full max-w-lg overflow-y-auto rounded-t-3xl bg-paper-50 p-6 sm:rounded-3xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="font-display text-2xl font-black text-ink-950">
              {existing ? "Edit" : KIND_NEW_TITLE[kind]}
            </h3>
            <p className="mt-1 text-sm text-ink-900/60">{KIND_BLURB[kind]}</p>
          </div>
          <button
            onClick={onDone}
            aria-label="Close"
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-ink-950/5 text-ink-900/60 hover:bg-ink-950/10"
          >
            ✕
          </button>
        </div>
        ...
        <div className="mt-5 flex flex-col gap-4">
          <label>
            <span className="mb-1 block text-[10px] font-black uppercase tracking-widest text-ink-900/40">
            ...
            />
            {kind !== "news" && (
              <span className="mt-1 block text-xs text-ink-900/45">
                {title.length}/60 —{" "}
                {kind === "promo"
          ...
          <label>
            ...
            />
            <span className="mt-1 block text-[10px] font-semibold text-ink-900/40">
          ...
          <label className="flex cursor-pointer items-center gap-2.5 rounded-xl bg-ink-950/[0.03] px-3 py-2.5">
            <input
              ...
              className="h-4 w-4 shrink-0 accent-accent-200"
            />
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex-1 rounded-2xl bg-ink-950 py-3 font-display text-lg font-black text-paper-50 transition-colors hover:bg-ink-900 disabled:bg-ink-950/15 disabled:text-ink-900/40"
            >
              {pending ? "Saving…" : "Save"}
            </button>
...
function DeleteButton({ id, onDone }: { id: number; onDone: () => void }) {
  ...
  if (!asking) {
    return (
      <button
        ...
        className="rounded-2xl px-4 py-3 text-sm font-bold text-brand-700 transition-colors hover:bg-brand-700 hover:text-paper-50"
      >
        Delete
      </button>
    );
  ...
  return (
    <button
      ...
      disabled={pending}
      className="rounded-2xl bg-brand-700 px-4 py-3 text-sm font-black text-paper-50 disabled:opacity-60"
    >
