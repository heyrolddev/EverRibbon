...
function MethodToggle({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
      <div className="min-w-0">
        <p className="font-display text-lg font-bold text-ink-950">{title}</p>
        <p className="text-sm text-ink-900/60">{blurb}</p>
      </div>
      <button
        type="button"
        onClick={onToggle}
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <section className="flex flex-col gap-4 rounded-2xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="flex flex-col gap-1.5">
              <span className="text-xs font-bold uppercase tracking-widest text-ink-900">
                Account name
              </span>
              <input
                value={gcashName}
          ...
          {/* Part payment */}
          <div className="flex flex-col gap-3 rounded-xl bg-paper-50 p-4 ring-1 ring-ink-950/10">
            ...
            {downEnabled && (
              <label className="flex flex-col gap-1.5">
                ...
                <span className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-sm font-semibold text-ink-900/60">
                    % of the total
                  </span>
                </span>
                <span className="text-[11px] text-ink-900/50">
                  On a {moneyRound(500)} order that&apos;s{" "}
                  <b>
                    {moneyRound(Math.round((500 * (Number(downPercent) || 0)) / 100))} now
                  </b>{" "}
                  and{" "}
                  {moneyRound(500 - Math.round((500 * (Number(downPercent) || 0)) / 100))} on
                  handover.
                </span>
              </label>
            )}
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative h-28 w-28 shrink-0 overflow-hidden rounded-xl bg-paper-50 ring-1 ring-ink-950/10">
              {qrUrl ? (
                <Image src={qrUrl} alt="GCash QR code" fill sizes="112px" className="object-contain" />
              ) : (
                <span className="grid h-full w-full place-items-center text-xs font-semibold text-ink-900/40">
                  No QR yet
                </span>
              )}
            </div>
            <div>
              ...
              <button
                ...
                className="rounded-full bg-ink-950 px-4 py-2 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-700 disabled:opacity-60"
              >
                {qrUrl ? "Replace QR code" : "Upload QR code"}
              </button>
              <p className="mt-2 max-w-xs text-xs text-ink-900/55">
      ...
      <button
        ...
        className={`self-start rounded-full px-7 py-3 font-bold transition-colors disabled:opacity-60 ${
          saved ? "bg-ok-600 text-paper-50" : "bg-brand-700 text-paper-50 hover:bg-brand-800"
        }`}
      >
