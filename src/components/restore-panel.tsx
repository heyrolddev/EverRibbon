 ...
 *   the shop backup" comes back instantly rather than after an upload.
 *
...
export function RestorePanel() {
  ...
  async function pick(file: File | undefined) {
    ...
    if (file.size > 64 * 1024 * 1024) {
      setError("That file is over 64MB — that isn't a ${brand.name} backup.");
      return;
    }
  ...
  return (
    <section className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10 sm:p-8">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <label className="cursor-pointer rounded-full bg-ink-950 px-5 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-700">
          Choose backup file
          <input
            ref={input}
            type="file"
      ...
      {result && result.error === null && (
        <div className="mt-5 rounded-2xl bg-paper-50 p-5 ring-1 ring-ink-950/10">
          <p className="font-display text-lg font-black text-ink-950">
            {rowsBack.toLocaleString()} rows restored
            {result.exportedAt && (
              <span className="ml-2 text-sm font-semibold text-ink-900/60">
            ...
            <div className="mt-4">
              ...
              <ul className="mt-2 flex flex-col gap-1.5">
                {failedTables.map((o: TableOutcome) => (
                  <li key={o.table} className="text-sm text-ink-900/75">
          ...
          <details className="mt-4">
            ...
            <ul className="mt-2 grid gap-1 sm:grid-cols-2">
              {result.outcomes.map((o) => (
                ...
                >
                  <code className="font-mono text-ink-900/75">{o.table}</code>
                  <span
                    className={`shrink-0 tabular-nums ${
                      o.error ? "text-brand-700" : "text-ink-900/55"
                    }`}
                  >
                    {o.restored}/{o.rows}
                  </span>
      ...
      {asking && (
        ...
        >
          <div className="flex flex-col gap-4">
            {preview && (
              <div className="rounded-2xl bg-paper-50 p-4 ring-1 ring-ink-950/10">
                ...
                <ul className="mt-3 grid gap-x-4 gap-y-1 sm:grid-cols-2">
                  {Object.entries(preview.counts).map(([table, n]) => (
                    ...
                    >
                      <code className="font-mono text-ink-900/70">{table}</code>
                      <span className="shrink-0 tabular-nums text-ink-900/55">
                        {n.toLocaleString()}
                      </span>
                    </li>
                  ))}
            ...
            {preview?.kind === "legacy" && (
              <div className="rounded-2xl bg-accent-300/25 px-4 py-3">
                ...
                <p className="mt-2 text-sm text-ink-900/75">
                  Products arrive <strong>hidden</strong> from the customer menu,
            ...
            {preview && preview.dropped.length > 0 && (
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-ink-900/50">
                  Not carried over
                </p>
                <ul className="mt-1.5 flex flex-col gap-1">
                  {preview.dropped.map((d) => (
                    <li key={d} className="text-xs text-ink-900/65">
                      • {d}
                    </li>
                  ))}
                </ul>
            ...
            <p className="rounded-xl bg-accent-300/25 px-4 py-3 text-sm font-semibold text-ink-950">
