...
export function MyAccount({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {pendingRole && (
        <section className="rounded-3xl bg-ink-950 p-6 text-paper-50 ring-2 ring-accent-200 sm:p-8">
          <p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-accent-200">
            <span className="h-px w-6 bg-accent-200/60" />
            Waiting for you
          </p>
          ...
          <p className="mt-3 max-w-xl text-sm leading-relaxed text-paper-100/80">
            {ROLE_BLURBS[pendingRole]}
          </p>
          <p className="mt-3 max-w-xl text-sm leading-relaxed text-paper-100/60">
            Nothing changes until you accept.
          </p>

          {offerError && (
            <p className="mt-4 rounded-xl bg-brand-700 px-4 py-2.5 text-sm font-semibold">
              {offerError}
            </p>
          )}
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              ...
              className="rounded-full bg-accent-200 px-5 py-2.5 text-sm font-black text-ink-950 transition-colors hover:bg-accent-300 disabled:opacity-60"
            >
            ...
            <button
              ...
              className="rounded-full px-5 py-2.5 text-sm font-bold text-paper-100/70 transition-colors hover:text-paper-50 disabled:opacity-60"
            >
              No thanks
            </button>
          </div>
      ...
      <section className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10 sm:p-8">
        ...
        <div className="mt-6 max-w-sm">
          ...
          {phoneError && (
            <p className="mt-3 rounded-xl bg-brand-700 px-4 py-2.5 text-sm font-semibold text-paper-50">
              {phoneError}
            </p>
          )}
          ...
          <div className="mt-4 flex items-center gap-3">
            <button
              ...
              className="rounded-full bg-ink-950 px-5 py-2.5 text-sm font-bold text-paper-50 transition-colors hover:bg-brand-700 disabled:opacity-50"
            >
              {savingPhone ? "Saving…" : "Save number"}
            </button>
            {saved && (
              <span className="text-sm font-semibold text-ok-700">Saved</span>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="rounded-3xl border-2 border-dashed border-ink-950/15 p-6 sm:p-8">
        ...
        <p className="mt-3 max-w-xl text-sm text-ink-900/60">
