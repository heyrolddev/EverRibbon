...
export function StaffToday({
  ...
  orders: ServiceOrder[];
  waitingLeads: number;
  /** Products the shelf can no longer make, or nearly can't. */
    ...
    <div className="flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            href="/admin/counter"
            className="rounded-full bg-ok-600 px-6 py-3 text-sm font-bold text-paper-50 transition-transform hover:scale-105"
          >
            Ring up a sale →
          </Link>
          {queue.length > 0 && (
          ...
          {waitingLeads > 0 && (
            <Link
              href="/admin/inbox"
              className="rounded-full bg-accent-200 px-6 py-3 text-sm font-bold text-ink-950 transition-transform hover:scale-105"
            >
          ...
          <ul className="mt-5 flex flex-col gap-2">
            {queue.map((o) => {
              ...
              return (
                <li
                  key={o.id}
                  className={`flex items-center justify-between gap-4 rounded-2xl border-l-4 bg-paper-100 px-5 py-3 ring-1 ring-ink-950/10 ${
                    tone?.rail ?? "border-ink-900"
                  }`}
                >
      ...
      {/* What to stop promising. A product that can't be made is worth knowing
