...
const labelClass =
  "flex flex-col gap-2 text-xs font-bold uppercase tracking-widest text-ink-900";
...
export function CheckoutForm({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const result = await placeOrder({
        items: items.map((i) => ({ productId: i.productId, qty: i.qty, name: i.name })),
        contactName,
        contactPhone,
        fulfillment,
        notes,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      {/* Order summary */}
      <div className="rounded-3xl bg-paper-100 p-6 ring-1 ring-ink-950/10">
        <p className="text-xs font-bold uppercase tracking-widest text-ink-900/60">
          Your order
        </p>
      ...
      <label className={labelClass}>
        Mobile number <span className="text-brand-700">*required</span>
        <input
          required
          inputMode="tel"
          value={contactPhone}
          ...
          className={fieldClass}
        />
        <span className="text-[11px] font-medium normal-case tracking-normal text-ink-900/50">
      ...
      {isDelivery && (
        <div className="flex flex-col gap-4 rounded-3xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
          ...
          {savedAddress && (
            <button
              ...
              className={`flex items-start gap-3 rounded-2xl p-4 text-left ring-2 transition-colors ${
                usingSaved
                  ? "bg-ok-50 ring-ok-600"
                  : "bg-paper-50 ring-ink-950/10 hover:ring-brand-700"
              }`}
            >
              <span className="mt-0.5 text-lg">📍</span>
              <span className="min-w-0 flex-1">
                <span className="block text-xs font-bold uppercase tracking-widest text-ink-900/55">
              ...
              {usingSaved && (
                <span className="shrink-0 text-sm font-black text-ok-700">✓</span>
              )}
            </button>
          )}
          ...
          <div>
            <p className="mb-2 text-xs font-bold uppercase tracking-widest text-ink-900">
              Pin your exact location <span className="text-brand-700">*required</span>
            </p>
            <MapPicker
              value={pin}
              onChange={(next) => {
          ...
          {pinWarning && (
            <p className="rounded-2xl bg-accent-50 px-5 py-3 text-sm font-semibold text-ink-900 ring-1 ring-accent-200/60">
              ⚠︎ {pinWarning}
            </p>
          )}
              ...
              {quote.ok ? (
                quote.waived ? (
                  <>
                    ...
                    {money(Number(delivery.free_over))} 🎉
                  </>
                ) : (
                  <>
                    ~{quote.km} km away · delivery fee <strong>{money(quote.fee)}</strong>
                  </>
                )
              ) : (
                quote.reason
          ...
          {delivery.notice && (
            <p className="text-xs text-ink-900/60">{delivery.notice}</p>
          )}
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 rounded-3xl bg-ink-950 px-6 py-5 text-paper-50">
        ...
        {isDelivery && (
          <div className="flex items-center justify-between text-sm text-paper-100/70">
            ...
            <span>{quote?.ok ? (fee === 0 ? "Free" : money(fee)) : "—"}</span>
          </div>
        )}
        <div className="mt-1 flex items-center justify-between border-t border-paper-50/15 pt-3">
          <span className="font-display text-lg font-bold">Total</span>
          <span className="font-display text-2xl font-black text-accent-200">
            {money(grandTotal)}
          </span>
        </div>
        {effectiveMethod === "gcash" && plan === "downpayment" && payments.downpayment_enabled && (
          <div className="mt-1 flex items-center justify-between border-t border-paper-50/15 pt-3 text-sm">
            <span className="text-paper-100/70">
              Send now ({payments.downpayment_percent}%)
            </span>
            <span className="font-bold text-paper-50">
              {money(amountDueNow(grandTotal, "downpayment", payments.downpayment_percent))}
              <span className="ml-2 font-normal text-paper-100/60">
                · {money(grandTotal - amountDueNow(grandTotal, "downpayment", payments.downpayment_percent))} on handover
              </span>
            </span>
          </div>
        )}
      ...
      {confirming && (
        <div className="flex flex-col gap-4 rounded-3xl bg-accent-50 p-6 ring-2 ring-accent-200">
          <div>
          ...
          <ul className="flex flex-col gap-1.5">
            {items.map((i) => (
              <li
                key={i.productId}
                className="flex items-center justify-between gap-3 rounded-xl bg-paper-50 px-4 py-2.5 text-sm"
              >
                <span className="min-w-0 truncate font-semibold text-ink-950">
                  {i.qty}× {i.name}
                </span>
                <span className="shrink-0 font-mono text-ink-900/70">
                  {money(i.price * i.qty)}
                </span>
              </li>
            ))}
          ...
          <div className="rounded-xl bg-paper-50 px-4 py-3">
            <p className="flex items-center justify-between text-sm">
              <span className="text-ink-900/70">Total to pay</span>
          ...
          <div className="rounded-xl bg-paper-50 px-4 py-3">
            <p className="text-xs font-bold uppercase tracking-widest text-ink-900/55">
              When
            </p>
            ...
            {isDelivery ? (
              <>
                ...
                {/* Coordinates mean nothing to a customer — what they need
                    ...
                    far it is. */}
                {pin && (
                  <p className="mt-1 text-[11px] font-semibold text-ok-700">
                    ✓ Pin dropped
          ...
          <button
            ...
            className="self-start text-sm font-bold text-ink-900 hover:text-brand-700"
          >
