 ...
 * the number.
 */
export function MenuAvailability({ products }: { products: AdminMeal[] }) {
  return (
    <AdminSearch
      rows={products}
      searchText={(m) =>
...
function Row({ product }: { product: AdminMeal }) {
  ...
  const [on, setOn] = useState(product.is_available);
  ...
  function toggle() {
    ...
    startTransition(async () => {
      const res = await setMealAvailability(product.id, next);
      if (res.error) {
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-2xl px-4 py-3 ring-1 transition-colors ${
        on ? "bg-paper-100 ring-ink-950/10" : "bg-brand-700/10 ring-brand-700/25"
      }`}
    >
