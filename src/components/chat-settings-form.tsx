...
export function ChatSettingsForm({
  ...
  return (
    <div className="rounded-3xl bg-paper-100 p-5 ring-1 ring-ink-950/10">
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        <span className="shrink-0 text-ink-900/40">{open ? "▲" : "▼"}</span>
      </button>
        ...
        <form onSubmit={submit} className="mt-4 flex flex-col gap-3">
          <label className="flex flex-col gap-1.5 text-xs font-bold uppercase tracking-widest text-ink-900">
            ...
            <input
              ...
              className="rounded-2xl border-2 border-ink-950/15 bg-paper-50 px-4 py-2.5 text-sm font-normal outline-none focus:border-brand-700"
            />
            <span className="text-[11px] font-medium normal-case tracking-normal text-ink-900/55">
          ...
          {error && (
            <p className="rounded-2xl bg-brand-50 px-4 py-2.5 text-sm font-semibold text-brand-800">
              {error}
            </p>
          )}
          ...
          <button
            ...
            className={`self-start rounded-full px-6 py-2.5 text-sm font-bold transition-colors disabled:opacity-60 ${
              saved ? "bg-ok-600 text-paper-50" : "bg-brand-700 text-paper-50"
            }`}
          >
