...
import { MealEditor, type AdminMeal } from "@/components/product-editor";
...
export function AdminMenuList({
  ...
  // Matched against every category a product carries, not just its first — the
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Above the list, because it is about the list as a whole and because
          ...
          untagged products this panel exists to find. */}
      <BulkCategory products={products} categories={categories} />

      {activeCategory && (
        <div className="flex flex-wrap items-center gap-3 rounded-2xl bg-ink-950/5 px-4 py-2.5">
          <p className="text-sm font-bold text-ink-950">
            Showing {shown.length} product{shown.length === 1 ? "" : "es"} in{" "}
            {activeCategory}
          </p>
          ...
          <button
            ...
            className="rounded-full bg-paper-50 px-3 py-1 text-xs font-bold text-ink-900/70 ring-1 ring-ink-950/10 transition-colors hover:bg-ink-950 hover:text-paper-50"
          >
          ...
          filtered.length === 0 ? (
            ...
            <ul className="flex flex-col gap-4">
              {filtered.map((product) => (
                <li key={product.id}>
                  <MealEditor product={product} categories={categories} />
                </li>
              ))}
            </ul>
          )
