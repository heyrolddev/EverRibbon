 ...
 * trade as the shop assistant on the customer side, made for the same reason.
...
export const TOPICS: GuideTopic[] = [
  // ---------------- Money -------------------------------------------
  {
    ...
    answer:
      ...
      "• Revenue − material cost (COGS) = gross profit\n" +
  ...
  },
  {
    ...
    triggers: ["gross profit", "gross", "cogs", "cost of goods", "material cost", "gastos sa sangkap"],
    where: { href: "/admin/money", label: "Costs & cash" },
    answer:
      "Gross profit is revenue minus what the food itself cost — the materials and the packaging, nothing else.\n\n" +
      "It is the number that tells you whether your PRICES are right. Net profit tells you whether your BUSINESS is right. A product can have a healthy gross profit and the shop still lose money, if rent is eating it.\n\n" +
      "COGS (cost of goods sold) is the material side of that. Every order records its own COGS at the moment it was sold, using the material prices at that time — so a later price rise does not quietly rewrite last month's profit.",
  },
  {
    id: "oe",
    group: "Money",
    ...
    answer:
      ...
      "So if your fixed costs are ${moneyRound(13000)} and you open 26 days, every trading day carries ${moneyRound(500)} before a single peso of profit is counted. Change the 'days open per month' setting and every OE and break-even number moves with it.",
  },
  {
    id: "break-even",
    group: "Money",
    ...
    answer:
      ...
      "• ÷ your margin (the share of each peso left after materials)\n" +
      "• ÷ days you open per month\n\n" +
      "Dividing by the margin is the step people skip. If only 60 centavos of each peso survives the materials, you have to sell ${moneyRound(1.67)} to cover every ${moneyRound(1)} of rent — not ${moneyRound(1)}.",
  },
  {
    id: "cash",
    group: "Money",
  ...
  },
  {
    ...
    answer:
      ...
      "The total shown is the unpaid remainder — amount minus what has already been collected — of everything not yet settled. Part payments are handled, so somebody who owed ${moneyRound(500)} and has paid ${moneyRound(200)} counts as ${moneyRound(300)} not ${moneyRound(500)} and not zero.\n\n" +
...

  {
    ...
    answer:
      ...
      "2. STOCK — Inventory → a material → Count. Type what is actually on the shelf and it shows the difference AND what that difference cost you. Materials walking out shows up here before it shows up in the money.\n\n" +
  ...
  // ---------------- The kitchen -------------------------------------
  {
    id: "product-margin",
    group: "The kitchen",
    question: "What does each product actually earn?",
    triggers: ["product cost", "margin", "food cost", "recipe cost", "magkano kita sa", "per product", "costing", "how much does a product earn"],
    numbers: "dish_margin",
    where: { href: "/admin/costing", label: "Product costs" },
    answer:
      "Product costs works out what a plate costs to make, from its recipe: every material at its current price, plus the packaging if it leaves the stall.\n\n" +
      "Two numbers come out of it:\n" +
      "• What you keep — price minus cost, in pesos\n" +
      "• Food cost % — the cost as a share of the price\n\n" +
      "The rule of thumb for a stall is to keep food cost at or under 30%. Above 40% and the product is working for the supplier, not for you. A product with no recipe yet shows as uncosted rather than as free — an unknown cost is not a zero cost.",
  },
  {
    id: "menu-engineering",
    group: "The kitchen",
    question: "What do star, plowhorse, puzzle and dog mean?",
    triggers: ["star", "plowhorse", "puzzle", "dog", "menu engineering", "which product should i push", "alin ang benta"],
    where: { href: "/admin/analytics", label: "Analytics" },
    answer:
      "Every product gets sorted by two things: how often it sells, and how much it keeps.\n\n" +
  ...
  },
  {
    ...
    where: { href: "/admin/inventory", label: "Inventory" },
    answer:
      "Stock moves by itself as you sell. Ring up a product and its recipe is deducted from the materials; cancel it and the stock comes back.\n\n" +
      "'Makeable servings' is the useful number: for each product, how many more you could make with what is on the shelf right now. It is limited by whichever material runs out first — twenty portions of noodles and two eggs means two servings, not twenty.\n\n" +
      "When a product falls to zero makeable servings it is marked sold out on the website automatically, so nobody orders what you cannot cook.",
  },
  {
    id: "restock",
    group: "The kitchen",
    question: "How do I record a delivery / restock?",
    triggers: ["restock", "delivery of supplies", "bumili", "pumalengke", "bought materials", "add stock", "resupply", "supplier", "record a delivery", "delivery of materials", "bought stock", "received stock", "new stock", "papalitan ng stock"],
    where: { href: "/admin/inventory", label: "Inventory" },
    answer:
      "Inventory → find the material → Restock.\n\n" +
      ...
      "That new price becomes the cost used for products made from then on. Orders already sold keep the price they were sold at — a price rise should not rewrite last week's profit.\n\n" +
  ...
  },
  {
    ...
    where: { href: "/admin/inventory", label: "Inventory" },
    answer:
      "Inventory → the material → Waste. Say how much and why (spoiled, dropped, staff product).\n\n" +
  ...
  },
  {
    ...
    answer:
      ...
      "Every product can have packaging attached — a box, a cup, a bag — and an order can have packaging of its own, charged once for the whole order rather than per product. Both are added to cost only when the order is take-out or delivery.\n\n" +
      "That is why the same product can show two different margins. Dine-in keeps more, and now you can see exactly how much more.",
  },
  ...
  },
  {
    ...
    answer:
      "Counter is the till. Tap the products, and the order is recorded exactly like an online one — so stock moves, profit is counted, and it shows in the day's takings.\n\n" +
  ...
  },
  {
    ...
    answer:
      ...
      "Cancelling puts the materials back on the shelf. That is the point of doing it here rather than just ignoring the order.",
  },
  {
    id: "sold-out",
    group: "Every day",
    question: "How do I mark a product sold out?",
    triggers: ["sold out", "soldout", "ubos", "unavailable", "hide product", "out of stock", "wala na"],
    where: { href: "/admin/menu", label: "Menu" },
    answer:
      "Menu → the product → the availability switch. It disappears from the website immediately and comes back the same way.\n\n" +
      "It also happens by itself: when the materials for a product run out, it is marked sold out automatically. So the usual reason to do it by hand is something the system cannot know — the fryer is down, or you have simply stopped making it today.\n\n" +
  ...
  },
  {
    ...
    answer:
      ...
      "• NEWS — dated, opens to its own page. A closure, a new product\n" +
  ...
  },
  {
    ...
    triggers: ["inbox", "chat", "message", "reply", "sagot", "customer question", `ask ${brand.name.toLowerCase()}`],
    where: { href: "/admin/inbox", label: "Inbox" },
    answer:
      `Inbox holds every conversation from the website's chat. Ask ${brand.name} answers the easy ones by itself — hours, prices, delivery, where you are — and anything it cannot answer waits for you.\n\n` +
      `Take over a thread and type; the customer sees it live. If it is a question you will be asked again, use 'Teach this answer' and Ask ${brand.name} will handle it next time.`,
  },
  ...
  // ---------------- Setting up --------------------------------------
  {
    ...
    answer:
      ...
      `For a one-off, add a closure with its date instead of editing the week and having to remember to put it back. The website shows it, Ask ${brand.name} tells customers about it, and it undoes itself.`,
  },
  {
    id: "delivery",
    group: "Setting up",
  ...
  },
  {
    ...
    answer:
      "Backup downloads one file with everything: orders, menu, recipes, materials, shifts, promos, answers, customers.\n\n" +
  ...
  },
  {
    ...
    answer:
      `Answers is one list used in two places. Anything you write there, Ask ${brand.name} says word for word — and it says it before its own built-in answers, so it is also how you correct one it gets wrong.\n\n` +
