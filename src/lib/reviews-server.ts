...
/**
 * Published reviews, newest first, with author names, photos and product names
 * resolved.
 *
...
export async function getPublicReviews(limit = 50): Promise<ReviewSummary> {
  try {
    ...
    const { data, error } = await supabase
      .from("reviews")
      .select(
        "id, customer_id, author_name, source, product_id, rating, comment, shop_reply, created_at"
      )
    ...
    const mealIds = [...new Set(rows.map((r) => r.product_id).filter(Boolean))] as string[];
    ...
    const [authorsRes, mealsRes] = await Promise.all([
      ...
      mealIds.length
        ? supabase.from("products").select("id, name").in("id", mealIds)
