 ...
 * dialog. One place decides what a receipt says; three places show
 * it.
 *
 ...
 *
 * So the text is folded to plain ASCII before it is sent: the shop's currency
 * symbol becomes its ASCII stand-in, curly
...
function toPrinterAscii(text: string): string {
  return text
    .split(brand.currency.symbol).join(brand.currency.ascii)
    .replace(/[""]/g, '"')
    .replace(/['']/g, "'")
    .replace(/[—–]/g, "-")
    .replace(/…/g, "...")
...
const amount = (n: number) =>
  n.toLocaleString(brand.locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
