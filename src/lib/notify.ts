...
function subjectFor(status: OrderStatus, ref: string): string {
  switch (status) {
    case "confirmed":
      return `Your ${brand.name} order is confirmed (${ref})`;
    case "preparing":
      return `We have started your ${brand.name} order (${ref})`;
    case "ready":
      return `Your ${brand.name} order is ready (${ref})`;
    case "out_for_delivery":
      return `Your ${brand.name} order is on the way (${ref})`;
    case "cancelled":
      return `Your ${brand.name} order was cancelled (${ref})`;
    default:
      return `Update on your ${brand.name} order (${ref})`;
  }
}
...
export async function notifyNewOrder(orderId: string): Promise<void> {
  ...
  try {
    ...
    await pushToStaff({
      title: `New order · ${moneyRound(total)}`,
      body: `${who} · ${how}${when}`,
      url: "/admin/orders",
