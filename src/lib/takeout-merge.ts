...
export type MergeLine = { refType: "inv" | "production_run"; refId: string; qty: number; label: string };
...
export type MergePlan = {
  rows: MergeRow[];
  skipped: string[];
  /** Products on the menu now, and how many would be left after. */
  before: number;
  after: number;
  error: string | null;
};
...
export async function planTakeoutMerge(): Promise<MergePlan> {
  ...
  // marks products sold out mid-service. If working out the suggestion throws
...
async function computePlan(): Promise<MergePlan> {
  ...
  const [products, lines, ing, bat] = await Promise.all([
    db.from("products").select("id, name, is_public, is_available"),
    db.from("product_materials").select("product_id, ref_type, ref_id, qty"),
    db.from("materials").select("id, name"),
    db.from("production_runs").select("id, name"),
  ]);
  if (products.error || lines.error) {
    return { ...empty, error: (products.error ?? lines.error)!.message };
  }

  type Product = { id: string; name: string; is_public: boolean; is_available: boolean };
  type Line = { product_id: string; ref_type: string; ref_id: string; qty: number };
  const allMeals = (products.data ?? []) as Product[];
  ...
  const label = new Map<string, string>([
    ...
    ...((bat.data ?? []) as { id: string; name: string }[]).map(
      (r) => [`production_run:${r.id}`, `${r.name} (production_run)`] as [string, string]
    ),
  ]);

  const linesFor = new Map<string, Line[]>();
  for (const l of allLines) {
    const list = linesFor.get(l.product_id) ?? [];
    list.push(l);
    linesFor.set(l.product_id, list);
  }

  // A dine-in product may share its name with another. Matching on a name that
  // isn't unique would attach one product's packaging to a different product, so
  ...
  // off the screen. (A "(T.O)" product hidden for some other reason is off the
  ...
  for (const twin of pending) {
    ...
    if (base === undefined) {
      skipped.push(`${twin.name} — no dine-in product of that name`);
      continue;
    }
    if (base === null) {
      skipped.push(`${twin.name} — more than one product shares that name`);
      continue;
    }
    ...
    for (const l of linesFor.get(twin.id) ?? []) {
      ...
      // uses more of a material keeps the difference, not the whole line.
 ...
 * Do it.
 *
 * Replaces each product's packaging rather than appending, so running this twice
 * cannot double the packaging on every product — the owner pressing the button
...
export async function applyTakeoutMerge(): Promise<{
  ...
  for (const r of plan.rows) {
    const { error: delErr } = await db
      ...
      .delete()
      .eq("product_id", r.baseId);
    if (delErr) {
      failed.push(`${r.baseName}: ${delErr.message}`);
      continue;
    }

    const { error: insErr } = await db.from("product_packaging").insert(
      r.packaging.map((l) => ({
        product_id: r.baseId,
        ref_type: l.refType,
        ref_id: l.refId,
