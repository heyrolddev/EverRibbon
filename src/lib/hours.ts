...
export function toMinutes(clock: string): number {
  // A string with no colon yields a one-element array, so the minutes half is
  // genuinely absent rather than merely unparseable — both end up at zero, but
  // only one of them is a case the compiler can see.
  const [h, m] = clock.split(":").map(Number);
  const hours = Number.isFinite(h) ? (h as number) : 0;
  const mins = Number.isFinite(m) ? (m as number) : 0;
