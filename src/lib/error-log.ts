...
export function isControlFlow(error: unknown): boolean {
  ...
  return prefix !== undefined && CONTROL_FLOW.includes(prefix);
}
 ...
 * usable at all.
 *
 * Digits are flattened to `#`, so "Material 4f21 not found" and
 * "Material 9c03 not found" are one problem rather than one problem per
 * material. That is the difference between a screen saying "this is broken
