...
async function netProfit(): Promise<string> {
  ...
  const table = sum([
    ...
    ["− Materials (COGS)", money(m.cogs)],
    ["= Gross profit", money(m.grossProfit)],
    [
      "− Operating expense",
      money(m.oeForWindow),
      `(${money(m.dailyOE)} a day × ${m.windowDays} ${m.windowDays === 1 ? "day" : "days"})`,
    ],
    ["= Before waste", money(beforeWaste)],
    ["− Waste", money(m.wasteForWindow)],
    ["= NET PROFIT", money(m.netProfit)],
  ]);
  return [
    ...
    table.lines[6],
    ``,
    `The ${money(m.dailyOE)} a day is your ${money(m.monthlyFixed)} of monthly fixed costs divided by the ${m.openDays} days a month you open. Every trading day carries that share whether it sells anything or not.`,
    m.netProfit < 0
...
async function breakEven(): Promise<string> {
  ...
  if (m.marginRatio === null || m.marginRatio <= 0) {
    return `Your fixed costs are ${money(
      m.monthlyFixed
    )} a month, but I can't work out a break-even yet: there aren't enough priced sales to know what share of each peso survives the materials. Cost a few products and sell a few, then ask again.`;
  }
  ...
  const table = sum([
    ["  Fixed costs a month", money(m.monthlyFixed)],
    ["+ Waste, as a month", money(m.monthlyWasteRate)],
    ["= To cover every month", money(m.monthlyFixed + m.monthlyWasteRate)],
  ]);
  return [
    `Your numbers:`,
    ``,
    ...
    table.lines[2],
    ``,
    `Of every peso you take, ${pct(m.marginRatio)} survives the materials.`,
    `So you must SELL ${money(m.monthlyFixed + m.monthlyWasteRate)} ÷ ${pct(
      m.marginRatio
    )} = ${money(monthlyTarget)} a month`,
    ``,
    `÷ ${m.openDays} days open = ${money(daily)} A DAY.`,
    ``,
    `You are averaging ${money(m.avgDailyRevenue)} a day ${overWindow(m.windowDays)} — ${
      gap >= 0
        ? `${money(gap)} above break-even.`
        : `${money(Math.abs(gap))} SHORT of it.`
    }`,
  ].join("\n");
}
...
async function cash(): Promise<string> {
  ...
  const table = sum([
    ["  Started with", money(m.cash.startedWith)],
    ["+ Cash sales since", money(takings)],
    ["+ Money put in", money(movedIn)],
    ["− Money taken out", money(movedOut)],
    ["= Should be in the drawer", money(m.cash.onHand)],
  ]);
  return [
...
async function utang(): Promise<string> {
  ...
  const lines = open
    ...
    .map(
      (r) =>
        `  • ${r.customer || "(no name)"} — ${money(r.amount - r.collected)}${
          r.collected > 0 ? `  (${money(r.amount)} less ${money(r.collected)} already paid)` : ""
        }`
    );
  return [
    `${open.length} unpaid utang, ${money(m.owed)} in total:`,
    ``,
    ...lines,
...
async function payback(): Promise<string> {
  ...
  const table = sum([
    ["  What you put in", money(m.assetTotal)],
    ["− Earned back since", money(m.payback.earned)],
    ["= Still to earn back", money(left), `(${Math.round(m.payback.pct)}% recovered)`],
  ]);
  return [
    ...
    ``,
    `"Earned back" is net — after materials, after fixed costs and after waste. It is deliberately the hard number, because the easy one would tell you the shop had paid for itself long before it had.`,
  ].join("\n");
}
...
async function dishMargin(): Promise<string> {
  ...
  const all = [...book.mealCosts.values()]
    ...
    .map((mc) => ({
      ...
      price: mc.product.price,
      cost: mc.cost,
      m: marginFor(mc.product.price, mc.cost, mc.costed),
      sold: volume.get(mc.product.id) ?? 0,
      // Priced, but with holes — a material with no purchase price makes
  ...
  if (solid.length === 0) {
    return [
      "I can't tell you what any product earns yet, and I'd rather say that than show you a number that isn't true.",
      "",
      shaky.length > 0
        ? `${shaky.length} ${
            shaky.length === 1 ? "product has a recipe" : "products have recipes"
          } but at least one material in ${
            shaky.length === 1 ? "it" : "them"
          } has no purchase price, so the cost would come out as ${moneyRound(0)} and the product would look like pure profit.`
        : "No product has a recipe yet.",
      "",
      "Fix it on Product costs: give every material a price by recording one restock at what you actually paid. That single hour is the highest-value thing you can do in here — every margin, every break-even and every profit figure depends on it.",
    ].join("\n");
  }
  ...
  return [
    ...
    ...top.map(
      (r) =>
        `  • ${r.name} — sells ${moneyRound(r.price)}, costs ${money(r.cost)}, you keep ${money(
          r.m.gross
        )} (${Math.round(r.m.foodCostPct ?? 0)}% food cost)`
    ),
    ``,
    `Food cost is the materials as a share of the price. Aim for 30% or under; over 40% and the product is working harder for your supplier than for you.`,
    ...
    shaky.length > 0
      ? `\n${shaky.length} ${
          shaky.length === 1 ? "product is" : "products are"
        } left out because a material in ${
        ...
        } ${moneyRound(0)} which would make ${
          shaky.length === 1 ? "it look" : "them look"
        } like pure profit.`
      : null,
    noRecipe > 0
      ? `\n${noRecipe} ${noRecipe === 1 ? "product has" : "products have"} no recipe at all yet.`
      : null,
  ]
    .filter((line) => line !== null)
    .join("\n");
...
async function stock(): Promise<string> {
  ...
  const nameOf = new Map([...book.mealCosts.values()].map((mc) => [mc.product.id, mc.product.name]));

  const rows = [...can.entries()]
    .map(([id, n]) => ({ name: nameOf.get(id) ?? "(deleted product)", can: n }))
    .sort((a, b) => a.can - b.can);

  if (rows.length === 0) {
    return "No product has a recipe yet, so I can't work out how many servings you could make. Once a product has materials, this becomes the most useful number on the Inventory screen.";
  }
  ...
  return [
    ...
    ``,
    out.length > 0
      ? `${out.length} ${out.length === 1 ? "product is" : "products are"} at zero, and marked sold out on the website automatically.`
      : `Nothing is at zero.`,
    low.length > 0
      ? `${low.length} more ${low.length === 1 ? "is" : "are"} down to three or fewer.`
      : null,
    ``,
    `Each number is limited by whichever material runs out first — twenty portions of noodles and two eggs is two servings, not twenty.`,
  ]
    .filter((line) => line !== null)
    .join("\n");
}
...
async function today(): Promise<string> {
  ...
  const table = sum([
    ...
    ["  Taken", money(revenue)],
    ["− Materials", money(cogs)],
    ["= Kept so far", money(revenue - cogs)],
