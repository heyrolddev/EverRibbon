...
export function moneyLine(m: MoneyState): string {
  if (m.settled && m.paid >= m.total) return `${money(m.total)} paid in full`;
  if (m.settled) return `${money(m.total)} · refunded`;
  if (m.partPaid) {
    return `${money(m.paid)} of ${money(m.total)} · ${money(m.balance)} still owed`;
  }
  if (m.awaitingCheck) return `${money(m.total)} sent — not checked yet`;
  return `${money(m.total)} unpaid`;
}
