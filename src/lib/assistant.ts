...
/**
 * The shop assistant answers from the shop's own data — no AI model, no API key,
 * nothing to pay for.
 *
 * The trade is deliberate: this can only answer the questions it recognises,
 * but it can never invent a product, quote a price the shop doesn't charge, or
...
async function readFacts(): Promise<Facts> {
  ...
  type Line = { qty: number; products: { name: string } | null };
  ...
  return {
    ...
    products: (mealsRes.data ?? []) as Product[],
    bestSeller,
    delivery: d
      ? {
...
function mealsMentioned(message: string, products: Product[]): Product[] {
  ...
  for (const product of products) {
    ...
    if (n.includes(name)) {
      hits.push({ product, score: name.length + 100 });
      continue;
    }
    ...
    if (matched.length > 0) {
      hits.push({ product, score: matched.join("").length });
    }
  }
...
function deliveryAnswer(f: Facts, tl: boolean): string {
  ...
  const free =
    d.freeOver > 0
      ? tl
        ? ` Libre na po ang delivery pag umabot ng ${money(d.freeOver)} ang order niyo!`
        : ` Delivery is free on orders over ${money(d.freeOver)}!`
      : "";

  return tl
    ? `Nagdedeliver po kami! ${money(d.baseFee)} po para sa unang ${d.baseKm} km, tapos ${money(d.perKm)} kada dagdag na km — hanggang ${d.maxKm} km po ang abot namin.${free} Lalabas po ang exactong fee sa checkout pag na-pin niyo ang location niyo.`
    : `Yes, we deliver! ${money(d.baseFee)} covers the first ${d.baseKm} km, then ${money(d.perKm)} per extra km, up to ${d.maxKm} km.${free} Your exact fee shows at checkout once you drop your pin.`;
}
...
function menuAnswer(f: Facts, tl: boolean): string {
  const available = f.products.filter((m) => m.is_available);
...
function priceAnswer(products: Product[], tl: boolean): string {
    ...
    .map((m) => {
      const out = m.is_available
        ...
        : tl
          ? " (ubos po ngayon)"
          : " (sold out right now)";
      return `${m.name} — ${money(m.price)}${out}`;
    })
    .join("\n");
...
function bestSellerAnswer(f: Facts, tl: boolean): string {
  ...
  return tl
    ? `Ang pinaka-order po sa amin ay ${product.name} — ${money(product.price)} lang po. ${product.description ?? "Subukan niyo po!"}`
    : `Our most-ordered product is ${product.name} at ${money(product.price)}. ${product.description ?? "Give it a try!"}`;
}
...
export async function askAssistant(history: ChatTurn[]): Promise<AssistantReply> {
  ...
  // --- specific product, with or without a price word -------------------------
  const named = mealsMentioned(message, facts.products);
  ...
  // --- a bare "how much" with no product named --------------------------------
  if (asksPrice) {
    return { text: menuAnswer(facts, tl), needsHuman: false };
  }
  ...
  if (has(n, ["salamat", "thank", "thanks", "sige po", "ok po", "okay po"])) {
    return {
      text: tl
        ? `Walang anuman po! Kita-kits sa ${brand.name} 🧡`
        : `Anytime! See you at ${brand.name} 🧡`,
      needsHuman: false,
    };
  }
